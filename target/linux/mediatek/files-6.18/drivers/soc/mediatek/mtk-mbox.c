		...
		void __iomem *send_status_reg, void __iomem *recv_status_reg)
{
	...
	n = snprintf(name, sizeof(name), "mbox%d", mbox);
	if (n < 0 || n >= sizeof(name))
		pr_err("mbox%d name init fail\n", mbox);
...
int mtk_mbox_probe(struct platform_device *pdev, struct mtk_mbox_device *mbdev,
		unsigned int mbox, unsigned int flag)
{
	struct mtk_mbox_info *minfo;
	char name[32];
	int n;
	...
	if (pdev) {
		/*base reg*/
		if (flag & MBOX_REG_RESOURCE_BASE) {
			n = snprintf(name, sizeof(name), "mbox%d_base", mbox);
			if (n < 0 || n >= sizeof(name))
				pr_err("mbox%d_base name init fail\n", mbox);
			res = platform_get_resource_byname(pdev, IORESOURCE_MEM, name);
			if (res == NULL) {
				pr_err("MBOX %d get mem resource fail\n", mbox);
			...
			if (resource_size(res))
				...
				pr_err("MBOX %d get invalid base size\n", mbox);
				goto mtk_mbox_probe_fail;
			}
		}

		/*init reg*/
		if (flag & MBOX_REG_RESOURCE_INIT) {
			n = snprintf(name, sizeof(name), "mbox%d_init", mbox);
			if (n < 0 || n >= sizeof(name))
				pr_err("mbox%d_init name init fail\n", mbox);
			res = platform_get_resource_byname(pdev, IORESOURCE_MEM, name);
		...
		/*set irq reg*/
		n = snprintf(name, sizeof(name), "mbox%d_set", mbox);
		if (n < 0 || n >= sizeof(name))
			pr_err("mbox%d_set name init fail\n", mbox);
		res = platform_get_resource_byname(pdev, IORESOURCE_MEM, name);
		...
		/*clear reg*/
		n = snprintf(name, sizeof(name), "mbox%d_clr", mbox);
		if (n < 0 || n >= sizeof(name))
			pr_err("mbox%d_clr name init fail\n", mbox);
		res = platform_get_resource_byname(pdev, IORESOURCE_MEM, name);
		...
		/*send status reg*/
		if (flag & MBOX_REG_RESOURCE_SEND) {
			n = snprintf(name, sizeof(name), "mbox%d_send", mbox);
			if (n < 0 || n >= sizeof(name))
				pr_err("mbox%d_send name init fail\n", mbox);
			res = platform_get_resource_byname(pdev, IORESOURCE_MEM, name);
		...
		/*recv status reg*/
		if (flag & MBOX_REG_RESOURCE_RECV) {
			n = snprintf(name, sizeof(name), "mbox%d_recv", mbox);
			if (n < 0 || n >= sizeof(name))
				pr_err("mbox%d_recv name init fail\n", mbox);
			res = platform_get_resource_byname(pdev, IORESOURCE_MEM, name);
