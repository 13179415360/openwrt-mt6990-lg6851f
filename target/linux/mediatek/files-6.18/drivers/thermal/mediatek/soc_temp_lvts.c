 ...
 *==================================================
 */
static int __maybe_unused lvts_raw_to_temp_v1(struct formula_coeff *co, unsigned int sensor_id,
...
static unsigned int __maybe_unused lvts_temp_to_raw_v1(struct formula_coeff *co, unsigned int sensor_id,
	int temp)
{
...
static void __maybe_unused device_identification_v1(struct lvts_data *lvts_data)
{
	struct device *dev = lvts_data->dev;
	unsigned int i, data;
	void __iomem *base;
...
static void lvts_wait_for_all_sensing_point_idle(struct lvts_data *lvts_data)
{
	...
	while (cnt < 50) {
		...
		if ((cnt + 1) % 10 == 0) {
                        dev_info(dev, "Cnt= %d LVTS TC %d, LVTSMSRCTL1[10,7,0] = %ld,%ld,%ld, LVTSMSRCTL1[10:0] = 0x%lx\n",
					cnt + 1, (temp >> 16),
					((temp & BIT(2)) >> 2),
					((temp & BIT(1)) >> 1),
					(temp & BIT(0)),
...
static int lvts_init(struct lvts_data *lvts_data)
{
	...
	ret = clk_prepare_enable(lvts_data->clk1);
	if (ret)
		dev_err(dev,
			"Error: Failed to enable lvts controller clock1: %d\n",
			ret);

	ret = clk_prepare_enable(lvts_data->clk2);
	if (ret)
		dev_err(dev,
			"Error: Failed to enable lvts controller clock2: %d\n",
			ret);

	ret = clk_prepare_enable(lvts_data->clk3);
	if (ret)
		dev_err(dev,
			"Error: Failed to enable lvts controller clock3: %d\n",
			ret);

	ret = clk_prepare_enable(lvts_data->clk4);
	if (ret)
		dev_err(dev,
			"Error: Failed to enable lvts controller clock4: %d\n",
			ret);
...
static void __maybe_unused check_cal_data_v1(struct lvts_data *lvts_data)
{
	struct device *dev = lvts_data->dev;
	struct sensor_cal_data *cal_data = &lvts_data->cal_data;
	struct tc_settings *tc = lvts_data->tc;
	...
	struct platform_device *pdev)
{
	...
	lvts_data->clk1 = devm_clk_get(dev, "lvts_clk1");
	if (IS_ERR(lvts_data->clk1))
		return PTR_ERR(lvts_data->clk1);
		
	lvts_data->clk2 = devm_clk_get(dev, "lvts_clk2");
	if (IS_ERR(lvts_data->clk2))
		return PTR_ERR(lvts_data->clk2);
		
	lvts_data->clk3 = devm_clk_get(dev, "lvts_clk3");
	if (IS_ERR(lvts_data->clk3))
		return PTR_ERR(lvts_data->clk3);
		
	lvts_data->clk4 = devm_clk_get(dev, "lvts_clk4");
	if (IS_ERR(lvts_data->clk4))
		return PTR_ERR(lvts_data->clk4);
	...
	for (i = 0; i < lvts_data->num_domain; i++) {
		...
		/*
		 * Newer kernels do not always populate an IORESOURCE_IRQ for this
		 * vendor-style MT6990 node.  Its DT interrupt specifiers are valid,
		 * so resolve them from OF directly.
		 */
		irq = of_irq_get(dev->of_node, i);
		if (irq <= 0) {
			dev_err(dev, "Failed to map irq, index %d: %d\n", i, irq);
...
static void lvts_close(struct lvts_data *lvts_data)
{
	...
	clk_disable_unprepare(lvts_data->clk1);
	clk_disable_unprepare(lvts_data->clk2);
	clk_disable_unprepare(lvts_data->clk3);
	clk_disable_unprepare(lvts_data->clk4);
}
...
#define SET_DEVICE_LOW_POWER_SINGLE_MODE_V4 (DEVICE_WRITE | RG_TSFM_CTRL_3 << 8	\
						| 0xF8)
static void __maybe_unused device_enable_and_init_v4(struct lvts_data *lvts_data)
{
	unsigned int i;
...
static void __maybe_unused device_enable_and_init_v5(struct lvts_data *lvts_data)
{
	unsigned int i;
...
static int __maybe_unused device_read_count_rc_n_v4(struct lvts_data *lvts_data)
{
...
static int __maybe_unused device_read_count_rc_n_v5(struct lvts_data *lvts_data)
{
 ...
 *==================================================
 */
 #define SET_TS_DIS_6980 (DEVICE_WRITE | RG_TSV2F_CTRL_0 << 8	\
						| 0xF1)
						
#define SET_LVTS_MANUAL_RCK_OPERATION_6980 (DEVICE_WRITE | RG_TSV2F_CTRL_6 << 8	| 0x00)

#define SET_COUNTING_WINDOW_47US2 (DEVICE_WRITE | RG_TSFM_CTRL_1 << 8 | 0x47)

#define TSV2F_CHOP_CKSEL_AND_TSV2F_EN_6980_1 (DEVICE_WRITE | RG_TSV2F_CTRL_2 << 8	\
						| 0x30)

#define TSV2F_CHOP_CKSEL_AND_TSV2F_EN_6980_2 (DEVICE_WRITE | RG_TSV2F_CTRL_3 << 8	\
						| 0x03)

#define TSBG_DEM_CKSEL_X_TSBG_CHOP_EN_6980 (DEVICE_WRITE | RG_TSV2F_CTRL_4 << 8	\
						| 0xFF)

#define SET_TS_RSV_6980 (DEVICE_WRITE | RG_TSV2F_CTRL_1 << 8 | 0x95)
#define SET_TS_CHOP_CTRL_6980 (DEVICE_WRITE | RG_TSV2F_CTRL_0 << 8 | 0xF1)
#define SET_TS_DIV_EN_6980 (DEVICE_WRITE | RG_TSV2F_CTRL_0 << 8 | 0xF5)
#define SET_VCO_RST_6980 (DEVICE_WRITE | RG_TSV2F_CTRL_0 << 8 | 0xFD)
#define COF_A_T_SLP_GLD 224280
#define COF_A_COUNT_R_GLD 14698
#define COF_A_CONST_OFS 280000
#define COF_A_OFS (COF_A_T_SLP_GLD - COF_A_CONST_OFS)
#define COF_A_T_SLP_GLD_HT 279280
#define COF_A_COUNT_R_GLD_HT 18302
#define COF_A_CONST_OFS_HT 170000
#define COF_A_OFS_HT (COF_A_T_SLP_GLD_HT - COF_A_CONST_OFS_HT)

static void mt6980_device_enable_and_init(struct lvts_data *lvts_data)
{
	unsigned int i;
	...
	for (i = 0; i < lvts_data->num_tc; i++) {
		...
		lvts_write_device(lvts_data, TSV2F_CHOP_CKSEL_AND_TSV2F_EN_6980_1, i);
		lvts_write_device(lvts_data, TSV2F_CHOP_CKSEL_AND_TSV2F_EN_6980_2, i);
		lvts_write_device(lvts_data, TSBG_DEM_CKSEL_X_TSBG_CHOP_EN_6980, i);
		lvts_write_device(lvts_data, SET_TS_RSV_6980, i);
		lvts_write_device(lvts_data, SET_TS_CHOP_CTRL_6980, i);
	}

	lvts_data->counting_window_us = 20;
}

static void mt6980_check_cal_data(struct lvts_data *lvts_data)
{
	struct device *dev = lvts_data->dev;
	struct sensor_cal_data *cal_data = &lvts_data->cal_data;
	struct tc_settings *tc = lvts_data->tc;
...
static void mt6980_update_coef_data(struct lvts_data *lvts_data)
{
	struct sensor_cal_data *cal_data = &lvts_data->cal_data;
	struct tc_settings *tc = lvts_data->tc;
	unsigned int i, j, s_index;
...
static int mt6980_device_read_count_rc_n(struct lvts_data *lvts_data)
{
	...
	for (i = 0; i < lvts_data->num_tc; i++) {
		base = GET_BASE_ADDR(i);
		lvts_write_device(lvts_data, SET_LVTS_MANUAL_RCK_OPERATION_6980, i);
		...
		for (j = 0; j < tc[i].num_sensor; j++) {
			...
			lvts_write_device(lvts_data, SET_DEVICE_SINGLE_MODE_V4, i);
			lvts_write_device(lvts_data, SET_TS_DIV_EN_6980, i);
			lvts_write_device(lvts_data, SET_VCO_RST_6980, i);
			lvts_write_device(lvts_data, SET_TS_DIV_EN_6980, i);
			udelay(10);
			...
			lvts_write_device(lvts_data, SET_TS_DIS_6980, i);
			data = lvts_read_device(lvts_data, 0x00, i);
			udelay(5);
...
#define MT6980_NUM_LVTS (ARRAY_SIZE(mt6980_tc_settings))

enum mt6980_lvts_domain {
	MT6980_AP_DOMAIN,
	MT6980_MCU_DOMAIN,
	MT6980_NUM_DOMAIN
};

enum mt6980_lvts_sensor_enum {
	MT6980_TS1_0,
	MT6980_TS1_1,
	MT6980_TS1_2,
	MT6980_TS1_3,
...
enum mt6980_lvts_controller_enum {
	MT6980_LVTS_MCU_CTRL0,
	MT6980_LVTS_MCU_CTRL1,
	MT6980_LVTS_AP_CTRL4,
	MT6980_LVTS_CTRL_NUM
};

static void mt6980_device_identification(struct lvts_data *lvts_data)
{
	struct device *dev = lvts_data->dev;
	unsigned int i, data;
	void __iomem *base;
	...
	for (i = 0; i < lvts_data->num_tc; i++) {
		...
		if  (tc[i].domain_index == MT6980_AP_DOMAIN)
			lvts_dev_id = 0x87 + i;
		else
			lvts_dev_id = 0x81 + i;
...
static void mt6980_efuse_to_cal_data(struct lvts_data *lvts_data)
{
	struct sensor_cal_data *cal_data = &lvts_data->cal_data;
	struct tc_settings *tc = lvts_data->tc;
	int i = 0;
	...
	cal_data->count_r[MT6980_TS1_0] = GET_CAL_DATA_BITMASK(1, 15, 0);
	cal_data->count_r[MT6980_TS1_1] = GET_CAL_DATA_BITMASK(1, 31, 16);
	cal_data->count_r[MT6980_TS1_2] = GET_CAL_DATA_BITMASK(2, 15, 0);
	cal_data->count_r[MT6980_TS1_3] = GET_CAL_DATA_BITMASK(2, 31, 16);

	cal_data->count_r[MT6980_TS2_0] = GET_CAL_DATA_BITMASK(3, 15, 0);
	cal_data->count_r[MT6980_TS2_1] = GET_CAL_DATA_BITMASK(3, 31, 16);
	cal_data->count_r[MT6980_TS2_2] = GET_CAL_DATA_BITMASK(4, 15, 0);
	cal_data->count_r[MT6980_TS2_3] = GET_CAL_DATA_BITMASK(4, 31, 16);

	cal_data->count_r[MT6980_TS9_0] = GET_CAL_DATA_BITMASK(7, 15, 0);
	cal_data->count_r[MT6980_TS9_1] = GET_CAL_DATA_BITMASK(7, 31, 16);
	cal_data->count_r[MT6980_TS9_2] = GET_CAL_DATA_BITMASK(8, 15, 0);
	cal_data->count_r[MT6980_TS9_3] = GET_CAL_DATA_BITMASK(8, 31, 16);

	cal_data->count_rc[MT6980_LVTS_MCU_CTRL0] = GET_CAL_DATA_BITMASK(9, 23, 0);
	cal_data->count_rc[MT6980_LVTS_MCU_CTRL1] = GET_CAL_DATA_BITMASK(10, 23, 0);
	cal_data->count_rc[MT6980_LVTS_AP_CTRL4] = GET_CAL_DATA_BITMASK(12, 23, 0);
}

static struct tc_settings mt6980_tc_settings[] = {
	[MT6980_LVTS_MCU_CTRL0] = {
		.domain_index = MT6980_MCU_DOMAIN,
		.addr_offset = 0x0,
		.num_sensor = 4,
		.sensor_map = {MT6980_TS1_0, MT6980_TS1_1, MT6980_TS1_2, MT6980_TS1_3},
		.device_id = {0, 1, 2, 3},
	...
	[MT6980_LVTS_MCU_CTRL1] = {
		...
		.addr_offset = 0x100,
		.num_sensor = 4,
		.sensor_map = {MT6980_TS2_0, MT6980_TS2_1, MT6980_TS2_2, MT6980_TS2_3},
		.device_id = {0, 1, 2, 3},
	...
	[MT6980_LVTS_AP_CTRL4] = {
		.domain_index = MT6980_AP_DOMAIN,
		.addr_offset = 0x400,
		.num_sensor = 4,
		.sensor_map = {MT6980_TS9_0, MT6980_TS9_1, MT6980_TS9_2, MT6980_TS9_3},
		.device_id = {0, 1, 2, 3},
...
static struct lvts_data mt6980_lvts_data = {
	.num_domain = MT6980_NUM_DOMAIN,
	.num_tc = MT6980_LVTS_CTRL_NUM,
	.tc = mt6980_tc_settings,
	.num_sensor = MT6980_NUM_TS,
	.ops = {
		.device_identification = mt6980_device_identification,
		.efuse_to_cal_data = mt6980_efuse_to_cal_data,
		.device_enable_and_init = mt6980_device_enable_and_init,
		.device_enable_auto_rck = device_enable_auto_rck_v4,
		.device_read_count_rc_n = mt6980_device_read_count_rc_n,
		...
		.check_cal_data = mt6980_check_cal_data,
		.update_coef_data = mt6980_update_coef_data,
	},
...
static const struct of_device_id lvts_of_match[] = {
	{
		.compatible = "mediatek,mt6980-lvts",
		.data = (void *)&mt6980_lvts_data,
	},
	{
	},
};
