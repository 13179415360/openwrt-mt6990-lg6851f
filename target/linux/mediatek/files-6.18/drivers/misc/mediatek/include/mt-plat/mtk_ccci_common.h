...
enum {
	...
	// colgin @{
	SYSMSGSV_POWEROFF_NOTIFY = 0x122,
	SYSMSGSV_MD_THERM_NOTIFY = 0x123,
	SYSMSGSV_MD_BC_CDP_NOTIFY = 0x124,/*USB notify to enable/disable DP/DM driver*/
    // @}
	CCMSG_ID_SYSMSGSVC_LOWPWR_APSTS_NOTIFY = 0x128,
	// colgin @{
	SYSMSGSV_PCIE_PM_NOTIFY = 0x129,
	// @}
...
enum SMEM_USER_ID {
	...
	// colgin @{
	SMEM_USER_USB_DATA,
	/* CDMR:Crash Dump Memory Region/MIDR:Modem Internals Dump Region */
	SMEM_USER_MD_CDMR,
...
/* LK load modem, export by ccci util for AMMS_v3 */
...
// colgin @{
int mtk_ccci_open_port(char *port_name);/* return fd/index */
...
struct ThinMdDhl {
    uint8_t aee_mini_dump_flag;
    uint8_t midr_dump_flag;
    uint8_t version;
    uint8_t rm_sensitive_data_flag;
    uint32_t memory_dump_timeout;
    uint8_t dump_level;
    uint8_t memory_dump_flag;
    uint8_t reserved[6];
    uint64_t redump_ratio;
};

struct ThinMdDrdiRuntimeData{
    uint16_t version;
    uint16_t combine_idx;
    uint16_t gpioPinNum[10];
    uint16_t gpioPinValue[10];
    uint16_t auxadcVolt1;
    uint16_t auxadcValue1;
    uint16_t auxadcVolt2;
    uint16_t auxadcValue2;
    uint16_t error_code;
    uint16_t rat_idx[11];
};
