...
#define HSAPIF_CCCI_LOG_LEVEL HSAPIF_CCCI_LOG_CRITICAL_UART
//#define HSAPIF_CCCI_LOG_LEVEL HSAPIF_CCCI_LOG_ALL_MOBILE
#endif

unsigned int ext_ccci_debug_enable = HSAPIF_CCCI_LOG_LEVEL;
...
static struct ccci_ring_buffer ccci_log_buf;

int ext_ccci_log_write(const char *fmt, ...)
{
	va_list args;
	...
	count = scnprintf(temp_log, CCCI_LOG_MAX_WRITE,
						...
						current->comm);
	if (count < 0) {
		pr_notice("[cldma0/util][%s] %d failed to format temp_log\n",
					__func__, count);
		kfree(temp_log);
		temp_log = NULL;
		return -ENODEV;
...
int ext_ccci_log_write_raw(unsigned int set_flags, const char *fmt, ...)
{
	va_list args;
	int write_len, first_half;
	unsigned long flags;
	...
	if (set_flags & CCCI_DUMP_TIME_FLAG) {
		...
		count = scnprintf(temp_log, CCCI_LOG_MAX_WRITE,
					...
					this_cpu);
		if (count < 0) {
			pr_notice("[cldma0/util][%s] 1. %d failed to format temp_log\n",
					__func__, count);
			kfree(temp_log);
			temp_log = NULL;
			return -ENODEV;
	...
	if (set_flags & CCCI_DUMP_CURR_FLAG) {
		...
		if (count < 0) {
			pr_notice("[cldma0/util][%s] 2. %d failed to format temp_log\n",
					__func__, count);
			kfree(temp_log);
			temp_log = NULL;
			return -ENODEV;
...
static unsigned int ccci_log_poll(
		struct file *fp,
...
#define CCCI_NORMAL_BUF			(4096*64)
#define CCCI_REPEAT_BUF			(4096*16)
#define CCCI_HISTORY_BUF		(4096*4)
#define CCCI_REG_DUMP_BUF		(4096*128)
...
struct ccci_user_ctlb {
	unsigned int read_idx[2][HSAPIF_CCCI_DUMP_MAX];
	unsigned int sep_cnt1[2][HSAPIF_CCCI_DUMP_MAX];
...
static struct buffer_node node_array[2][HSAPIF_CCCI_DUMP_MAX+1] = {
	{
		{&init_setting_ctlb[0], CCCI_INIT_SETTING_BUF,
		0, HSAPIF_CCCI_DUMP_INIT},
		{&boot_up_ctlb[0], CCCI_BOOT_UP_BUF,
		CCCI_DUMP_ATTR_RING, HSAPIF_CCCI_DUMP_BOOTUP},
		{&normal_ctlb[0], CCCI_NORMAL_BUF,
		CCCI_DUMP_ATTR_RING, HSAPIF_CCCI_DUMP_NORMAL},
		{&repeat_ctlb[0], CCCI_REPEAT_BUF,
		CCCI_DUMP_ATTR_RING, HSAPIF_CCCI_DUMP_REPEAT},
		{&reg_dump_ctlb[0], CCCI_REG_DUMP_BUF,
		CCCI_DUMP_ATTR_RING, HSAPIF_CCCI_DUMP_MEM_DUMP},
		{&history_ctlb[0], CCCI_HISTORY_BUF,
		CCCI_DUMP_ATTR_RING, HSAPIF_CCCI_DUMP_HISTORY},
		{&ke_dump_ctlb[0], 32*1024,
		CCCI_DUMP_ATTR_RING, HSAPIF_CCCI_DUMP_REGISTER},
	},
	{
...
int ext_ccci_dump_write(int md_id, int buf_type,
	unsigned int flag, const char *fmt, ...)
{
	...
	/* parameter check */
	if (unlikely(md_id >= HSAPIF_MAX_MD_NUM))
		return -1;
	if (unlikely(md_id < 0))
		md_id = 0;
	if (unlikely(buf_type >= HSAPIF_CCCI_DUMP_MAX))
		return -2;
	if (unlikely(buf_type < 0))
		return -3;
	buf_id = buff_bind_md_id[md_id];
	if (unlikely(buf_id >= 2))
		return -4;
...
EXPORT_SYMBOL(ext_ccci_dump_write);
...
static void format_separate_str(char str[], int type)
{
	...
	switch (type) {
	case HSAPIF_CCCI_DUMP_INIT:
		sep_str = "[0]INIT LOG REGION";
		break;
	case HSAPIF_CCCI_DUMP_NORMAL:
		sep_str = "[0]NORMAL LOG REGION";
		break;
	case HSAPIF_CCCI_DUMP_BOOTUP:
		sep_str = "[0]BOOT LOG REGION";
		break;
	case HSAPIF_CCCI_DUMP_REPEAT:
		sep_str = "[0]ERROR LOG REGION";
		break;
	case HSAPIF_CCCI_DUMP_MEM_DUMP:
		sep_str = "[0]MEM DUMP LOG REGION";
		break;
	case HSAPIF_CCCI_DUMP_HISTORY:
		sep_str = "[0]HISTORY LOG REGION";
		break;
	case HSAPIF_CCCI_DUMP_REGISTER:
	...
	for (i = 0; i < 2; i++) {
		...
		if (curr < 64) {
			...
			if (ret == 0) {
				...
				pr_notice("[cldma0/util]dump copy to ser fail%d[-1]\n",
					ret);
		}
		...
		while (node_ptr->ctlb_ptr != NULL) {
			...
			if (curr < 64) {
				...
				if (ret == 0) {
					...
					pr_notice(
					"[cldma0/util]dump copy to ser fail%d[-2]\n",
						ret);
			}
			...
			if ((ptr->attr & CCCI_DUMP_ATTR_RING) == 0) {
				...
				if (ret == 0) {
					...
					pr_notice(
					"[cldma0/util]dump copy to ser fail%d\n",
						ret);
				...
				if (read_pos + available > ptr->buf_size) {
					...
					if (ret == 0) {
						...
						pr_notice("[cldma0/util]dump copy to ser fail%d[2]\n",
									ret);
				} else {
					ret = copy_to_user(&buf[has_read],
							ptr->buffer + read_pos,
					...
					if (ret == 0) {
						...
						pr_notice("[cldma0/util]dump copy to ser fail%d[3]\n",
									ret);
				}
			}
		}
...
static unsigned int ccci_dump_poll(
		struct file *fp,
		struct poll_table_struct *poll)
{
...
static void ccci_dump_buffer_init(void)
{
	...
	ccci_dump_proc = proc_create("ccci_cldma_dump", 0660, NULL, &ccci_dump_fops);
	if (ccci_dump_proc == NULL) {
		pr_notice("[cldma0/util]fail to create proc entry for dump\n");
		return;
	}
	...
	for (i = 0; i < 5; i++) {
		...
		if (j >= 2)
			continue;
		if (get_plat_capbility(HSAPIF_MD_SYS1 + i)) {
			buff_bind_md_id[HSAPIF_MD_SYS1 + i] = j;
			buff_en_bit_map |= 1<<j;
			md_id_bind_buf_id[j] = HSAPIF_MD_SYS1 + i + 1;
			j++;
		}
	}
		...
		while (node_ptr->ctlb_ptr != NULL) {
			...
			if (buff_en_bit_map & (1<<i) && node_ptr->init_size) {
				...
				if (ptr->buffer != NULL) {
					...
					pr_notice("[cldma0/util]fail to allocate buff index %d\n",
						node_ptr->index);
			}
			node_ptr++;
		}
...
int ext_ccci_get_dump_buf_usage(char buf[], int size)
{
	int ret = 0;
	int i;
...
void ext_ccci_util_mem_dump(int md_id, int buf_type, void *start_addr, int len)
{
	...
	if (curr_p == NULL) {
		ext_ccci_dump_write(md_id, buf_type, 0, "start_addr <NULL>\n");
		return;
	}
	if (len == 0) {
		ext_ccci_dump_write(md_id, buf_type, 0, "len [0]\n");
		return;
	}

	ext_ccci_dump_write(md_id, buf_type, 0, "Base: %p\n", start_addr);
	/* Fix section */
	for (i = 0; i < _16_fix_num; i++) {
		ext_ccci_dump_write(md_id, buf_type, 0,
				"%03X: %08X %08X %08X %08X\n",
...
EXPORT_SYMBOL(ext_ccci_util_mem_dump);

void ext_ccci_util_cmpt_mem_dump(int md_id, int buf_type,
...
int ext_ccci_event_log(const char *fmt, ...)
{
	va_list args;
	unsigned int write_len = 0;
	char *temp_log;
	...
	tv.tv_usec = tv64.tv_nsec/NSEC_PER_USEC;
	tv_android = tv;
...
EXPORT_SYMBOL(ext_ccci_event_log);

int ext_cci_event_log_cpy(char buf[], int size)
{
	unsigned long flags;
...
void ext_ccci_log_init(void)
{
#if 0  // Not used currently.
	struct proc_dir_entry *ccci_log_proc;

	ccci_log_proc = proc_create("ccci_cldma_log", 0444, NULL, &ccci_log_fops);
...
void ccci_get_aee_buffer(unsigned long *vaddr, unsigned long *size)
{
...
EXPORT_SYMBOL(ccci_get_aee_buffer);

void ccci_get_md_aee_buffer(unsigned long *vaddr, unsigned long *size)
{
...
EXPORT_SYMBOL(ccci_get_md_aee_buffer);
