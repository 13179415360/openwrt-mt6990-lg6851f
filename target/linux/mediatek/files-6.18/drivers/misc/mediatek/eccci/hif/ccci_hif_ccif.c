	...
	int blocking, int *result)
{
	...
	if (!md_ctrl) {
		CCCI_ERROR_LOG(-1, TAG, "ccci_hif_get_by_id fail!\n");
		return -EFAULT;
	}

	per_md_data = ccci_get_per_md_data(md_ctrl->md_id);
	if (!per_md_data) {
		CCCI_ERROR_LOG(md_ctrl->md_id, TAG, "ccci_get_per_md_data fail!\n");
		return -EFAULT;
	}
 ...
 OUT:
	...
	*result = count;
	CCCI_DEBUG_LOG(md_ctrl->md_id, TAG,
		"%s Q%d len=%d channel=%d seq_num=%d %d pkg, ret=%d\n",
		__func__, queue->index, (skb == NULL ? -1 : skb->len),
		(ccci_h == NULL ? -1 : ccci_h->channel),
		(ccci_h == NULL ? -1 : ccci_h->seq_num),
		count, ret);
	spin_lock_irqsave(&queue->rx_lock, flags);
	...
	struct sk_buff *skb, int skb_from_pool, int blocking)
{
	...
	md_cap = ccci_md_get_cap_by_id(md_ctrl->md_id);
	per_md_data = ccci_get_per_md_data(md_ctrl->md_id);
	if (!per_md_data)
		return -CCCI_ERR_GET_NULL_POINTER;
 ...
 retry:
	...
	if (ccci_ringbuf_writeable(md_ctrl->md_id,
		...
		CCCI_DEBUG_LOG(md_ctrl->md_id, TAG,
			"%s Q%d len=%d channel=%d seq_num=%d ch_pkt_cnt=%ld\n",
			__func__, qno, skb->len, ccci_h->channel, ccci_h->seq_num,
			md_ctrl->traffic_info.logic_ch_pkt_cnt[ccci_h->channel]);
...
static int md_ccif_exp_ring_buf_init(struct md_ccif_ctrl *md_ctrl)
{
	int i = 0;
	unsigned char *buf;
	int bufsize = 0;
	...
	for (i = 0; i < QUEUE_NUM; i++) {
		...
		ringbuf =
			...
			tx_exp_buffer_size_up_95[i]);
#else
		ringbuf = (struct ccci_ringbuf *)(buf + sizeof(int) * 2);
#endif
		if (ringbuf == NULL) {
			CCCI_ERROR_LOG(md_ctrl->md_id, TAG,
	...
	void __iomem *baseB, struct md_ccif_ctrl *md_ctrl)
{
...

	{
		...
		regmap_write(md_ctrl->plat_val.infra_ao_base,
			reset_offset, 1 << reset_bit);
		/*clear reset bit*/
		regmap_write(md_ctrl->plat_val.infra_ao_base,
			clear_offset, 1 << reset_bit);
	}
...
static int ccif_late_init(unsigned char hif_id)
{
	...
	if (ch_id != 0) {
		...
		ccif_write32(ccif_ctrl->ccif_ap_base,
			APCCIF_ACK, ch_id & 0xFFFF);
		CCCI_NORMAL_LOG(ccif_ctrl->md_id, TAG,
			"%s ch_id = 0x%lX 0x%X\n", __func__, ccif_ctrl->channel_id, ch_id);
		/* igore exception queue */
		if (ch_id >> RINGQ_BASE) {
			ccif_ctrl->traffic_info.isr_cnt++;
			ccif_ctrl->traffic_info.latest_isr_time
				= local_clock();
	...
	ret = request_irq(ccif_ctrl->ap_ccif_irq0_id, md_ccif_isr,
			ccif_ctrl->ap_ccif_irq0_flags, "CCIF_AP_DATA", ccif_ctrl);
	CCCI_NORMAL_LOG(ccif_ctrl->md_id, TAG,
		"request CCIF_AP_DATA IRQ0(%d)\n", ccif_ctrl->ap_ccif_irq0_id);
	if (ret) {
	...
	ret = request_irq(ccif_ctrl->ap_ccif_irq1_id, md_cd_ccif_isr,
			...
			ccif_ctrl);
	CCCI_NORMAL_LOG(ccif_ctrl->md_id, TAG,
		"request CCIF_AP_DATA IRQ1(%d)\n", ccif_ctrl->ap_ccif_irq1_id);
	if (ret) {
		CCCI_ERROR_LOG(ccif_ctrl->md_id, TAG,
...
static int ccif_start(unsigned char hif_id)
{
...
#else
	CCCI_NORMAL_LOG(ccif_ctrl->md_id, TAG, "skip md_ccif_sram_reset\n");
#endif
...
#else
	CCCI_NORMAL_LOG(ccif_ctrl->md_id, TAG, "skip md_ccif_reset_queue EXP\n");
#endif
	...
	md_ccif_reset_queue(CCIF_HIF_ID, 1);
#else
	CCCI_NORMAL_LOG(ccif_ctrl->md_id, TAG, "skip md_ccif_reset_queue NORMAL\n");
#endif
	...
	ccci_reset_ccif_hw(ccif_ctrl->md_id, AP_MD1_CCIF,
		...
		ccif_ctrl->ccif_md_base, ccif_ctrl);
#else
	CCCI_NORMAL_LOG(ccif_ctrl->md_id, TAG, "skip ccci_reset_ccif_hw\n");
#endif
...
static int ccif_hif_hw_init(struct device *dev, struct md_ccif_ctrl *md_ctrl)
{
	...
	md_ctrl->md_pcore_pccif_base =
		ioremap(MD_PCORE_PCCIF_BASE, 0x20);
...
#else
	CCCI_NORMAL_LOG(-1, TAG,
		"skip request_irq CCIF_AP_DATA IRQ0(%d) in ccif_hif_hw_init\n",
		md_ctrl->ap_ccif_irq0_id);
#endif
	return 0;
...
#if IS_ENABLED(CONFIG_MTK_LOW_POWER_MODULE)
	wake_events = plat_mmio_read(SPM_BK_WAKE_EVENT);
	CCCI_NOTICE_LOG(-1, TAG, "[%s] wake_events=0x%x\n", __func__, wake_events);
	if ((wake_events & WAKE_EVENT_CCIF0)) {
		ccif_ch = ccif_read32(ccif_ctrl->ccif_ap_base, APCCIF_RCHNUM);
		CCCI_NORMAL_LOG(-1, TAG,
			"CCIF 0 Wake up: channel_id == 0x%x\n", ccif_ch);
		ccif_ctrl->wakeup_ch = ccif_ch;
...
static int ccci_ccif_hif_init(struct platform_device *pdev,
	unsigned char hif_id, unsigned char md_id)
{
	...
	//register_syscore_ops(&ccif_pm_ops);
...
static int ccci_hif_ccif_probe(struct platform_device *pdev)
{
	int ret;
