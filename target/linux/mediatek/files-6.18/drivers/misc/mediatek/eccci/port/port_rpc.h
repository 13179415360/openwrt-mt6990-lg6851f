...
struct rpc_buffer {
	...
	u32 para_num;
	/*
	 * The RPC payload occupies the remainder of the skb.  The vendor 5.15
	 * driver used a GNU zero-length array here; on 6.18 that makes fortify
	 * treat every payload copy as a write past a zero-byte field.  Model the
	 * wire-format tail as a real flexible array so object-size checking sees
	 * the enclosing dynamically-sized skb buffer correctly.
	 */
	u8 buffer[];
} __packed;
