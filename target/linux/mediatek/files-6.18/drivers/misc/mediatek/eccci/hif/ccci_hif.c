	...
	int queue_index, struct ccci_header *msg, int is_dropped)
{
	...
	if (queue_index < 0 ||
		(dir == OUT &&
			(queue_index >= MAX_TXQ_NUM ||
			(tinfo->tx_history_ptr[queue_index] < 0 || tinfo->tx_history_ptr[queue_index] >= PACKET_HISTORY_DEPTH))) ||
		(dir == IN &&
			(queue_index >= MAX_RXQ_NUM ||
			(tinfo->rx_history_ptr[queue_index] < 0 || tinfo->rx_history_ptr[queue_index] >= PACKET_HISTORY_DEPTH)))) {
		CCCI_ERROR_LOG(-1, CORE, "%s-%d: invalid queue_index = %d with dir = %d\n",
			__func__, __LINE__, queue_index, dir);
		return;
	}
