...
#endif
#ifdef _DPMAIF_MED_SUPPORT_
int dummyDrbBase0 = 10; //dummy drb base for MED's tx queue 0
int dummyDrbBase1 = 11; //dummy drb base for MED's tx queue 1
#ifdef _DPMAIF_GEN98_CODA_V2
int dummyDrbBase5 = 55; //dummy drb base for MED's tx queue 5
#endif
static int disable_rx_pitnat_log = 1;
module_param(disable_rx_pitnat_log, uint, 0644);
MODULE_PARM_DESC (disable_rx_pitnat_log, "disable rx_pitnat_log debugging");
#if defined(CONFIG_MTK_TINYSYS_MEDMCU_SUPPORT)
uint32_t msg_med_ccci[2]; //dummy msg for ipi usage
static DEFINE_MUTEX(scp_A_notify_mutex);
atomic_t med_mcu_ack_flag;
struct wakeup_source *push_wakelock;
struct wakeup_source *transmit_wakelock;
struct wakeup_source *transmit_wakelock_timer;
#endif
#endif
		...
		enum MODEM_DUMP_FLAG flag, void *buff, int length)
{
	...
	/*For disable net speed monitor feature*/
...
unsigned int ccci_skb_drop_count = 0;
#if defined(_DPMAIF_GEN98_CODA_V2) && defined(_DPMAIF_MED_SUPPORT_) && \
	defined(DPMAIF_MEDHW_RXIRQ)
static atomic_t med_rx_irq_count = ATOMIC_INIT(0);
#endif
#if DPMAIF_TRAFFIC_MONITOR_INTERVAL
static void dpmaif_traffic_monitor_func(struct timer_list *t)
{
	...
	CCCI_REPEAT_LOG(hif_ctrl->md_id, TAG,
		"net Rx ISR %lu.%06lu, active %d, dp %u\n",
		(unsigned long)tinfo->latest_isr_time, isr_rem_nsec / 1000,
		hif_ctrl->rxq[0].que_started, ccci_skb_drop_count);
	for (i = 0; i < DPMAIF_RXQ_NUM; i++) {
		...
		CCCI_REPEAT_LOG(hif_ctrl->md_id, TAG, "net RXq%d:%lu.%06lu\n",
			...
			q_rx_rem_nsec[i] / 1000);
		CCCI_NORMAL_LOG(hif_ctrl->md_id, TAG,
			"MT6990 DL q%d: pit rd/wr/rel=%u/%u/%u, bat rd/wr/rel=%u/%u/%u\n",
			i, hif_ctrl->rxq[i].pit_rd_idx,
			hif_ctrl->rxq[i].pit_wr_idx,
			hif_ctrl->rxq[i].pit_rel_rd_idx,
			hif_ctrl->rxq[i].bat_req.bat_rd_idx,
			hif_ctrl->rxq[i].bat_req.bat_wr_idx,
			hif_ctrl->rxq[i].bat_req.bat_rel_rd_idx);
	}
#if defined(_DPMAIF_GEN98_CODA_V2) && defined(_DPMAIF_MED_SUPPORT_) && \
	defined(DPMAIF_MEDHW_RXIRQ)
	CCCI_NORMAL_LOG(hif_ctrl->md_id, TAG,
		"MT6990 MEDHW DL irq=%d notif_w1c=0x%08x\n",
		atomic_read(&med_rx_irq_count),
		DPMA_READ_MED_INT(MEDHW_INT_NOTIF_W1C));
#endif
...
{
#ifdef _DPMAIF_GEN98_CODA_V2
	// channel id: 0~19, bit7 is used for ethernet PDU
	rxq->cur_chn_idx = msg_pit->channel_id & 0x1f;
#else
	...
	struct dpmaifq_normal_pit *pkt_inf_t)
{
...
#ifdef _DPMAIF_GEN98_CODA_
	frg_idx = (pkt_inf_t->buffer_id | ((pkt_inf_t->h_bid) << 13));
#else
	...
	bat_req->bat_rel_rd_idx = ringbuf_get_next_idx(bat_req->bat_size_cnt,
		pkt_inf_t->buffer_id | ((pkt_inf_t->h_bid) << 13), 1);
#else
	bat_req->bat_rel_rd_idx = ringbuf_get_next_idx(bat_req->bat_size_cnt,
...
#endif
	/*
	 * buffer_id is supplied by the modem/MED PIT.  Never use it to index
	 * the AP BAT shadow before validating it: an out-of-range descriptor
	 * must not turn into an arbitrary skb pointer or DMA unmap.
	 */
	if (unlikely(buffer_id >= rxq->bat_req.bat_size_cnt ||
		     buffer_id >= DPMAIF_DL_BAT_ENTRY_SIZE)) {
		CCCI_ERROR_LOG(dpmaif_ctrl->md_id, TAG,
			"invalid RX BID before BAT access: pit=%u bid=%u size=%u\n",
			rxq->pit_rd_idx, buffer_id, rxq->bat_req.bat_size_cnt);
		return DATA_CHECK_FAIL;
	}

	cur_skb_info = (struct dpmaif_bat_skb_t *)
		rxq->bat_req.bat_skb_ptr + buffer_id;
	new_skb = cur_skb_info->skb;
	data_len = pkt_inf_t->data_len;
	if (unlikely(!new_skb || !cur_skb_info->data_phy_addr ||
		     data_len > cur_skb_info->data_len)) {
		CCCI_ERROR_LOG(dpmaif_ctrl->md_id, TAG,
			"invalid RX BAT ownership: pit=%u bid=%u skb=%p dma=%pad len=%u/%u\n",
			rxq->pit_rd_idx, buffer_id, new_skb,
			&cur_skb_info->data_phy_addr, data_len,
			cur_skb_info->data_len);
		return DATA_CHECK_FAIL;
	}
...
static int ccci_skb_to_list(struct ccci_skb_queue *queue, struct sk_buff *newsk)
{
	...
	if (queue->skb_list.qlen < queue->max_len) {
		...
		ccci_skb_drop_count++;
		if (ccci_skb_drop_count % 120000 == 0) {
			CCCI_ERROR_LOG(-1, TAG,
				"%s: drop skb, %u\n", __func__, ccci_skb_drop_count);
		}
		...
		if (pkt_inf_t->ig == 0) {
			...
			if (pkt_inf_t->packet_type == DES_PT_MSG) {
				...
				if (pkt_inf_t->buffer_type != PKT_BUF_FRAG) {
					...
					/* && record ptr. */
					rxq->skb_idx = (pkt_inf_t->buffer_id |((pkt_inf_t->h_bid)<<13));
...
/* =======================================================
 *
 * Descriptions: MED RX part start
 *
 * ========================================================
 */

#if defined(_DPMAIF_MED_SUPPORT_)
static unsigned int dpmaifq_poll_med_rx_pit(struct dpmaif_rx_queue *rxq)
{
	unsigned int pit_cnt = 0;
	unsigned short sw_rd_idx = 0, hw_wr_idx = 0;
	...
	if (rxq->que_started == false)
		return pit_cnt;
	sw_rd_idx = rxq->pit_rd_idx;
	hw_wr_idx = drv_dpmaif_dl_get_med_wridx(rxq->index);

	pit_cnt = ringbuf_readable(rxq->pit_size_cnt, sw_rd_idx, hw_wr_idx);

	rxq->pit_wr_idx = hw_wr_idx;
	if (disable_rx_pitnat_log != 1) {
		CCCI_NORMAL_LOG(-1, TAG, "poll_med_rx_pit: pit_cnt:%u, sw_rd_idx:%u, hw_wr_idx:%u\n", pit_cnt, sw_rd_idx, hw_wr_idx);
        }
	return pit_cnt;
}

static int dpmaif_alloc_med_rx_buf(struct dpmaif_bat_request *bat_req,
		unsigned char q_num, unsigned int buf_cnt, unsigned int skb_idx, int blocking)
{
	struct sk_buff *new_skb = NULL;
	struct dpmaif_bat_t *cur_bat = NULL;
	struct dpmaif_bat_skb_t *cur_skb = NULL;
...
#ifdef DPMAIF_DEBUG_LOG_1
	CCCI_HISTORY_LOG(-1, TAG, "%s 1: 0x%x, 0x%x, %d\n", __func__,
		buf_cnt, bat_req->bat_size_cnt, skb_idx);
#endif
	if (buf_cnt == 0 || buf_cnt > bat_req->bat_size_cnt) {
		CCCI_ERROR_LOG(-1, TAG, "alloc_cnt is invalid !\n");
		return 0;
...
#ifdef DPMAIF_DEBUG_LOG
	/*CCCI_HISTORY_LOG(-1, TAG, "%s skb_idx:%u, idx: 0x%x -> 0x%x, 0x%x, 0x%x, 0x%x\n",
		__func__, skb_idx, bat_req->pkt_buf_sz, new_skb->len,
		new_skb->truesize, cur_bat_idx, sizeof(struct skb_shared_info));*/
	CCCI_HISTORY_LOG(-1, TAG, "%s skb_idx:%u, idx: 0x%x -> 0x%x, 0x%x, 0x%x\n",
		__func__, skb_idx, bat_req->pkt_buf_sz, new_skb->len,
		new_skb->truesize, cur_bat_idx);
#endif

	return ret;
}

static int dpmaif_alloc_med_rx_frag(struct dpmaif_bat_request *bat_req,
		unsigned char q_num, unsigned int buf_cnt, unsigned int skb_idx, int blocking)
{
	struct dpmaif_bat_t *cur_bat = NULL;
	struct dpmaif_bat_page_t *cur_page = NULL;
...
#ifdef DPMAIF_DEBUG_LOG
	CCCI_HISTORY_LOG(-1, TAG, "%s 1: 0x%x, 0x%x, skb_idx:%u\n", __func__,
		buf_cnt, bat_req->bat_size_cnt, skb_idx);
#endif
	if (buf_cnt == 0 || buf_cnt > bat_req->bat_size_cnt) {
		CCCI_ERROR_LOG(-1, TAG, "frag alloc_cnt is invalid !\n");
...
#ifdef _DPMAIF_GEN98_CODA_V2
// EthPDU +
extern struct eth_pdu_info_t get_eth_pdu_info(void);
// EthPDU -
#endif
...
#elif defined(CONFIG_NETFILTER_XT_TARGET_FLOWOFFLOAD)
extern void ccmni_set_ppe_rx_info(struct sk_buff *skb, u8 drop, u8 channel);
#elif defined(CONFIG_MTK_TINYSYS_MEDMCU_SUPPORT)
extern void ccmni_recycle_med_hnat_info(void);
#endif
#endif
static int dpmaif_send_med_skb_to_net(struct dpmaif_rx_queue *rxq,
	unsigned int skb_idx, int blocking)
{
	...
	struct lhif_header *lhif_h = NULL; /* for uplayer: port/ccmni */
	struct ccci_header ccci_h; /* for collect debug info. */
#if defined(_DPMAIF_GEN98_CODA_V2) && defined(CONFIG_HW_NAT)
	struct eth_pdu_info_t eth_pdu_info = get_eth_pdu_info();
#endif

	if (unlikely(skb_idx >= rxq->bat_req.bat_size_cnt ||
		     skb_idx >= DPMAIF_DL_BAT_ENTRY_SIZE)) {
		CCCI_ERROR_LOG(dpmaif_ctrl->md_id, TAG,
			"invalid RX BID before delivery: pit=%u bid=%u size=%u\n",
	...
	if (unlikely(!new_skb)) {
		CCCI_ERROR_LOG(dpmaif_ctrl->md_id, TAG,
			"RX BAT ownership lost before delivery: pit=%u bid=%u\n",
			rxq->pit_rd_idx, skb_idx);
		return DATA_CHECK_FAIL;
	}
...
#if defined(_DPMAIF_MED_SUPPORT_)
#if defined(CONFIG_HW_NAT)
		if (ppe_hook_rx_modem) {
			ppe_hook_rx_modem(new_skb, 1, 255);
		}
#elif defined(CONFIG_NETFILTER_XT_TARGET_FLOWOFFLOAD)
		ccmni_set_ppe_rx_info(new_skb, 1, 255);
#elif defined(CONFIG_MTK_TINYSYS_MEDMCU_SUPPORT)
		ccmni_recycle_med_hnat_info();
#endif
#endif
		dev_kfree_skb_any(new_skb);
		goto END;
...
#ifdef DPMAIF_DEBUG_LOG
		CCCI_HISTORY_LOG(-1, TAG, "before ppehook skb len: 0x%x+0x%x = 0x%x=> 0x%x\n",
			skb_headlen(new_skb), new_skb->data_len,
			new_skb->len, new_skb->truesize);
		//print_hex_dump(KERN_ERR, "\nskb_data1:", DUMP_PREFIX_OFFSET, 16, 1, new_skb->data, 64, false); //for debug
#endif
	...
	if (ppe_hook_rx_modem) {
#ifdef _DPMAIF_GEN98_CODA_V2
		if (eth_pdu_info.eth_mode && rxq->cur_chn_idx == eth_pdu_info.eth_id)
			ppe_result = ppe_hook_rx_modem(new_skb, 0, rxq->cur_chn_idx | 0x80);
		else
#endif
			ppe_result = ppe_hook_rx_modem(new_skb, 0, rxq->cur_chn_idx);
	}
#elif defined(CONFIG_NETFILTER_XT_TARGET_FLOWOFFLOAD)
	ccmni_set_ppe_rx_info(new_skb, 0, rxq->cur_chn_idx);
#elif defined(CONFIG_MTK_TINYSYS_MEDMCU_SUPPORT)
	ccmni_recycle_med_hnat_info();
#endif
#endif
	...
	if (ppe_result) {
#endif
		//print_hex_dump(KERN_ERR, "\nskb_data2:", DUMP_PREFIX_OFFSET, 16, 1, new_skb->data, 64, false); //for  debug
		/* md put the ccmni_index to the msg pkt,
		 * so we need push it by self. maybe no need
		 */
		lhif_h = (struct lhif_header *)(skb_push(new_skb,
...
END:
	cur_skb->skb = NULL;
	ret = dpmaif_alloc_med_rx_buf(&rxq->bat_req, rxq->index, 1, skb_idx, blocking);
	if (ret < 0) {
#ifdef DPMAIF_DEBUG_LOG
		CCCI_HISTORY_TAG_LOG(-1, TAG,
			"rx alloc fail: %d\n", ret);
		...
		CCCI_NORMAL_LOG(-1, TAG,
			"rx alloc fail: %d, %d\n", ret, skb_idx);
		if (ret == LOW_MEMORY_SKB) {
			rxq->skb_low_mem_idx = skb_idx;
			NOTIFY_RX_PUSH(rxq);
		}
		return ret;
	}

	if(drv_dpamif_med_bat_recycle_cmd(false, skb_idx, 1) < 0) {
#ifdef DPMAIF_DEBUG_LOG
		CCCI_HISTORY_TAG_LOG(-1, TAG,
			"rx med recycle fail: alloc ret = %d\n", ret);
#endif
		CCCI_NORMAL_LOG(-1, TAG,
			"rx med recycle fail: alloc ret = %d\n", ret);
		return DATA_CHECK_FAIL;
...
static int BAT_cur_med_bid_check(struct dpmaif_bat_request *bat_req,
	unsigned int cur_pit, unsigned int skb_idx)
{
	struct sk_buff *skb;
	int ret = 0;

	/* Validate the modem supplied BID before indexing the BAT shadow. */
	if (unlikely(skb_idx >= bat_req->bat_size_cnt ||
		     skb_idx >= DPMAIF_DL_BAT_ENTRY_SIZE)) {
		CCCI_ERROR_LOG(dpmaif_ctrl->md_id, TAG,
			"pkt(%u/%u): BID out of range (size=%u)\n",
			cur_pit, skb_idx, bat_req->bat_size_cnt);
		return DATA_CHECK_FAIL;
	}

	skb = ((struct dpmaif_bat_skb_t *)bat_req->bat_skb_ptr +
		skb_idx)->skb;
	if (unlikely(!skb)) {
		/*dump*/
		if (bat_req->check_bid_fail_cnt < 0xFFF)
...
static int dpmaif_get_rx_med_pkt(struct dpmaif_rx_queue *rxq,
	unsigned int skb_idx, int blocking,
	struct dpmaifq_normal_pit *pkt_inf_t)
{
	struct dpmaif_bat_request *bat_req = &rxq->bat_req;
	...
	/* check bid */
	bid_cnt = BAT_cur_med_bid_check(bat_req, rxq->pit_rd_idx, skb_idx);
	if (bid_cnt < 0) {
		ret = bid_cnt;
		return ret;
	}

	/*ret = dpmaif_alloc_med_rx_buf(&rxq->bat_req, rxq->index, 1, blocking);
	if (ret < 0) {
#ifdef DPMAIF_DEBUG_LOG
		CCCI_HISTORY_TAG_LOG(-1, TAG,
			"rx alloc fail: %d", ret);
	...
	if (ret < 0) {
		CCCI_ERROR_LOG(-1, TAG,
			"%s: dpmaif_rx_set_data_to_skb ret:%d\n",
			__func__, ret);
	}

	//drv_dpamif_med_bat_recycle_cmd(false,(pkt_inf_t->buffer_id|((pkt_inf_t->h_bid)<<13)),1);

	//For non-Frag pit entry, only get 1 pkg.
	return (ret < 0 ? ret : 1);
}

static int dpmaif_get_rx_med_frag(struct dpmaif_rx_queue *rxq,
	unsigned int skb_idx, int blocking,
	struct dpmaifq_normal_pit *pkt_inf_t)
{
	struct dpmaif_bat_request *bat_req = &rxq->bat_frag;
	int ret = 0;
	unsigned int pageId = (unsigned int) (pkt_inf_t->buffer_id |((pkt_inf_t->h_bid)<<13));

	if (pageId >= DPMAIF_DL_BAT_ENTRY_SIZE) {
		CCCI_NORMAL_LOG(-1, TAG, "dpmaif_get_rx_med_frag fail: %u,%u", pageId, skb_idx);
		return DATA_CHECK_FAIL;
	}
	if (pkt_inf_t->data_len > DPMAIF_HW_FRG_PKTBUF) {
		CCCI_NORMAL_LOG(-1, TAG, "dpmaif_get_rx_med_frag len fail: %u,%u, %d,%d",
			pageId, skb_idx, pkt_inf_t->data_len, DPMAIF_HW_FRG_PKTBUF);
		return DATA_CHECK_FAIL;
	}

	if (rxq->page_low_mem_idx < 0) {
		/* set frag data to skb_shinfo->frag_list */
		ret = dpmaif_set_rx_frag_to_skb(rxq, skb_idx, pkt_inf_t);
		if (ret < 0) {
			CCCI_HISTORY_TAG_LOG(-1, TAG, "dpmaif_set_rx_frag_to_skb fail: %d", ret);
			return ret;
		}
	}

	ret = dpmaif_alloc_med_rx_frag(bat_req, rxq->index, 1,
		(pkt_inf_t->buffer_id|((pkt_inf_t->h_bid)<<13)), blocking);
	if (ret < 0) {
		CCCI_HISTORY_TAG_LOG(-1, TAG, "rx med_rx_frag alloc fail: %d", ret);
		CCCI_NORMAL_LOG(-1, TAG, "rx med_rx_frag alloc fail: %d\n", ret);
		if (ret == LOW_MEMORY_BAT) {
			rxq->page_low_mem_idx = (pkt_inf_t->buffer_id|((pkt_inf_t->h_bid)<<13));
			NOTIFY_RX_PUSH(rxq);
		}
		return ret;
	}
	if (rxq->page_low_mem_idx >= 0) {
		rxq->page_low_mem_idx = -1;
	}

	if (drv_dpamif_med_bat_recycle_cmd(
		true,(pkt_inf_t->buffer_id|((pkt_inf_t->h_bid)<<13)),1) < 0) {
		CCCI_HISTORY_TAG_LOG(-1, TAG,
			"rx get_rx_med_frag fail: alloc ret = %d", ret);
...
static int dpmaifq_rel_rx_med_pit_entry(struct dpmaif_rx_queue *rxq,
			unsigned short rel_entry_num)
{
	unsigned short old_sw_rel_idx = 0, new_sw_rel_idx = 0,
		old_hw_wr_idx = 0;
	...
	ret = drv_dpamif_med_dl_pit_nat_release(rel_entry_num);

	return ret;
}

static int dpmaifq_rx_notify_med_hw(struct dpmaif_rx_queue *rxq,
	struct dpmaif_rx_hw_notify *notify_cnt)
{
	int ret = 0;

	if (notify_cnt->pit_cnt) {
		ret = dpmaifq_rel_rx_med_pit_entry(rxq, notify_cnt->pit_cnt);
		if (ret < 0) {
			CCCI_MEM_LOG_TAG(0, TAG,
				"dpmaif: update pit fail(128)\n");
			return ret;
...
static void dpmaif_dump_partial_pit_nat(struct dpmaif_rx_queue *rxq,
		unsigned int cur_pit)
{
	struct dpmaifq_normal_pit *pkt_inf_t;
	int dump_entry_offset_front = 5;
	int dump_pit_index = cur_pit - dump_entry_offset_front;
	int dump_entry_size = 9;
	void *dump_start;

	if (dump_pit_index < 0) {
		dump_pit_index = 0;
	}
	if (dump_pit_index + dump_entry_size >= rxq->pit_size_cnt) {
		dump_entry_size = rxq->pit_size_cnt - dump_pit_index - 1;
	}
	dump_start = rxq->pit_base + sizeof(struct dpmaifq_normal_pit)
			* dump_pit_index;
	pkt_inf_t = (struct dpmaifq_normal_pit *)rxq->pit_base +
			dump_pit_index;
	CCCI_NORMAL_LOG(-1, TAG,
			"%s: dump_pit_index:%d, cur_pit:%u, dump_entry_size:%d, 0x%llx\n", __func__,
			dump_pit_index, cur_pit, dump_entry_size, (unsigned long long)dump_start);
	ccci_util_mem_dump(-1, CCCI_DUMP_NORMAL, dump_start,
		(dump_entry_size *
		sizeof(struct dpmaifq_normal_pit)));
	CCCI_HISTORY_TAG_LOG(-1, TAG,
			"%s: dump_pit_index:%d, cur_pit:%u, dump_entry_size:%d, 0x%llx\n", __func__,
			dump_pit_index, cur_pit, dump_entry_size, (unsigned long long)dump_start);
	ccci_util_mem_dump(-1, CCCI_DUMP_HISTORY, dump_start,
		(dump_entry_size *
		sizeof(struct dpmaifq_normal_pit)));
}

static int dpmiaf_reallocate_rx_buf(struct dpmaif_rx_queue *rxq,
	unsigned int skb_idx, int blocking)
{
	struct dpmaif_rx_hw_notify notify_hw = {0};
	unsigned int pit_len = rxq->pit_size_cnt;
	...
	cur_pit = rxq->pit_rd_idx;
	CCCI_NORMAL_LOG(-1, TAG,
			"rx re-alloc blocking: %d, %d, %d\n", blocking, cur_pit, skb_idx);
...
#endif
		CCCI_NORMAL_LOG(-1, TAG,
			"rx re-alloc fail: %d, %d\n", ret, skb_idx);
		return ret;
	}
	//The rx buffer is allocated
	rxq->skb_low_mem_idx = -1;

	if(drv_dpamif_med_bat_recycle_cmd(false, skb_idx, 1) < 0) {
#ifdef DPMAIF_DEBUG_LOG
		CCCI_HISTORY_TAG_LOG(-1, TAG,
			"rx re-med recycle fail: alloc ret = %d\n", ret);
#endif
		CCCI_NORMAL_LOG(-1, TAG,
			"rx re-med recycle fail: alloc ret = %d\n", ret);
		return DATA_CHECK_FAIL;
	...
	rxq->pit_rd_idx = cur_pit;
	notify_hw.pit_cnt++;
	ret_hw = dpmaifq_rx_notify_med_hw(rxq, &notify_hw);
...
static int dpmaif_rx_med_start(struct dpmaif_rx_queue *rxq, unsigned short pit_cnt,
		int blocking, unsigned long time_limit)
{
	struct dpmaif_rx_hw_notify notify_hw = {0};
#ifndef PIT_USING_CACHE_MEM
	...
	if (rxq->skb_low_mem_idx >= 0) {
		CCCI_NORMAL_LOG(dpmaif_ctrl->md_id, TAG,
			"low mem: %d,%d", rxq->skb_low_mem_idx, blocking);
		ret = dpmiaf_reallocate_rx_buf(rxq, rxq->skb_low_mem_idx, blocking);
		CCCI_NORMAL_LOG(dpmaif_ctrl->md_id, TAG,
			"low mem ret: %d", ret);
		return ret;
	}
...
cache_start_addr = rxq->pit_phy_addr +
			...
			if (pkt_inf_t->packet_type == DES_PT_MSG) {
				...
				if (pkt_inf_t->buffer_type == PKT_BUF_FRAG
					...
					print_hex_dump(KERN_ERR, "\n[ccci][err]:",
						...
						pkt_inf_t, 16, false);
					dpmaif_dump_partial_pit_nat(rxq, cur_pit);
#ifdef DPMAIF_DEBUG_LOG
				...
				if (pkt_inf_t->buffer_type != PKT_BUF_FRAG) {
					...
					/* && record ptr. */
					rxq->skb_idx = (pkt_inf_t->buffer_id |((pkt_inf_t->h_bid)<<13));
					ret = dpmaif_get_rx_med_pkt(rxq,
						(unsigned int)rxq->skb_idx,
						blocking, pkt_inf_t);
					if (ret < 0) {
						CCCI_ERROR_LOG(dpmaif_ctrl->md_id, TAG,
							"rx_med_start ret=%d,", ret);
						if (ret == DATA_CHECK_FAIL) {
							print_hex_dump(KERN_ERR, "\n[ccci][err]:",
								DUMP_PREFIX_OFFSET, 16, 1,
								pkt_inf_t, 16, false);
							dpmaif_dump_partial_pit_nat(rxq, cur_pit);
					...
					/* skb->frag_list: add to frag_list */
					ret = dpmaif_get_rx_med_frag(rxq,
						(unsigned int)rxq->skb_idx,
					blocking, pkt_inf_t);
					if (ret < 0) {
						print_hex_dump(KERN_ERR, "\n[ccci][err]:",
				...
				if (pkt_inf_t->c_bit == 0) {
					...
					/* && data had rx. */
					ret = dpmaif_send_med_skb_to_net(rxq,
						rxq->skb_idx, blocking);
					if (ret < 0)
						break;
					recv_skb_cnt++;
					if ((recv_skb_cnt&0x7) == 0) {
	...
	if (ret_hw < 0 && ret == 0)
		ret = ret_hw;
#ifdef DPMAIF_DEBUG_LOG
	CCCI_NORMAL_LOG(-1, TAG, "end %s: pit:0x%x, 0x%x, 0x%x\n",
		__func__, rxq->pit_wr_idx, rxq->pit_rd_idx,
			rxq->pit_rel_rd_idx);
#endif
...
static int dpmaif_rx_med_data_collect(struct hif_dpmaif_ctrl *hif_ctrl,
		unsigned char q_num, int budget, int blocking)
{
	struct dpmaif_rx_queue *rxq = &hif_ctrl->rxq[q_num];
	unsigned int cnt, rd_cnt = 0, max_cnt = budget;
	...
	cnt = dpmaifq_poll_med_rx_pit(rxq);
	if (cnt) {
		max_cnt = cnt;
		rd_cnt = (cnt > budget && !blocking) ? budget:cnt;

		real_cnt = dpmaif_rx_med_start(rxq, rd_cnt, blocking, time_limit);
...
/*Communication functions with MEDMCU part*/
#if defined(_DPMAIF_MED_SUPPORT_) && defined(CONFIG_MTK_TINYSYS_MEDMCU_SUPPORT)
void dpmaif_ipi_rx_request(int id, void *data, unsigned int len)
{
	/*to-do*//*check this ipi is for ccci or not*/

	/*if (dpmaif_ctrl->dpmaif_state != HIFDPMAIF_STATE_PWRON) {
		CCCI_ERROR_LOG(-1, TAG, "ipi received when dpmaif not pwron\n");
		return;
	}*/
	if (disable_rx_pitnat_log != 1) {
		CCCI_NORMAL_LOG(-1, TAG, "ipi received\n");
	}
	if (dpmaif_ctrl->dpmaif_state != HIFDPMAIF_STATE_PWRON) {
		CCCI_ERROR_LOG(-1, TAG, "ipi received when dpmaif not pwron\n");
		return;
	...
	if (atomic_read(&dpmaif_ctrl->rxq[0].ipi_processing) != 0) {
		atomic_set(&dpmaif_ctrl->rxq[0].should_ipi_re_process, 1);
		smp_mb(); /* for cpu exec. */
#ifdef DPMAIF_DEBUG_LOG
		CCCI_HISTORY_TAG_LOG(-1, TAG,
			"ipi_rx return, busy");
	...
	atomic_set(&dpmaif_ctrl->rxq[0].ipi_processing, 1);
	smp_mb(); /* for cpu exec. */

	/*to-do, hold wake lock*/
	__pm_stay_awake(dpmaif_ctrl->rx_wakelock);

	// this is copy from dpmaif_irq_rx_done, no need for MED case
...
EXPORT_SYMBOL_GPL(dpmaif_ipi_rx_request);

static BLOCKING_NOTIFIER_HEAD(scp_A_notifier_list);

static void ccci_dpmaif_notify(int flag)
{
	mutex_lock(&scp_A_notify_mutex);
	if (flag == CCCI_DPMAIF_EVENT_READY) {
		CCCI_NORMAL_LOG(-1, TAG, "[Notify]ccci_dpmaif_notify:%d", flag);
		CCCI_HISTORY_TAG_LOG(-1, TAG, "[Notify]ccci_dpmaif_notify:%d", flag);
		blocking_notifier_call_chain(&scp_A_notifier_list
			, CCCI_DPMAIF_EVENT_READY, NULL);
	} else if (flag == CCCI_DPMAIF_EVENT_STOP) {
		CCCI_NORMAL_LOG(-1, TAG, "[Notify]ccci_dpmaif_notify:%d", flag);
		CCCI_HISTORY_TAG_LOG(-1, TAG, "[Notify]ccci_dpmaif_notify:%d", flag);
		blocking_notifier_call_chain(&scp_A_notifier_list
			, CCCI_DPMAIF_EVENT_STOP, NULL);
	}
...
void ccci_dpmaif_register_notify(struct notifier_block *nb)
{
	...
	CCCI_HISTORY_LOG(-1, TAG, "[Register]ccci_dpmaif_register_notify");

	if (dpmaif_ctrl != NULL &&
		dpmaif_ctrl->dpmaif_state == HIFDPMAIF_STATE_PWRON) {
		nb->notifier_call(nb, CCCI_DPMAIF_EVENT_READY, NULL);
	}
	mutex_unlock(&scp_A_notify_mutex);
}
EXPORT_SYMBOL_GPL(ccci_dpmaif_register_notify);
...
EXPORT_SYMBOL_GPL(ccci_dpmaif_unregister_notify);
...
#else
	ret = dpmaif_rx_med_data_collect(hif_ctrl, rxq->index, rxq->budget, 1);
#endif
...
#ifdef DPMAIF_DEBUG_LOG
			CCCI_HISTORY_TAG_LOG(-1, TAG, "ipi_re_process\n");
#endif
		} else {
			atomic_set(&rxq->ipi_processing, 0);
			smp_mb(); /* for cpu exec. */
		}
		__pm_relax(dpmaif_ctrl->rx_wakelock);
#endif
	}
	atomic_set(&rxq->rx_processing, 0);
}
...
#else
	ret = dpmaif_rx_med_data_collect(hif_ctrl, rxq->index, rxq->budget, 0);
#endif
...
#ifdef DPMAIF_DEBUG_LOG
		CCCI_HISTORY_TAG_LOG(-1, TAG, "relax wakelock\n");
#endif
		__pm_relax(dpmaif_ctrl->rx_wakelock);
#endif
	}
	...
	unsigned int release_cnt)
{
	...
	if (q_num == 0 || q_num == 1) {
#else
	if (q_num == 0 || q_num == 1 || q_num == 5) {
#endif
		CCCI_ERROR_LOG(dpmaif_ctrl->md_id, TAG,
			"rxq%d release tx error qNum\n", q_num);
		return 0;
	}
#endif
...
#endif
	/*Remove real_rel_cnt<0 condition, because real_rel_cnt is unsigned*/
...
#if defined(_DPMAIF_MED_SUPPORT_)
	__pm_wakeup_event(transmit_wakelock_timer, jiffies_to_msecs(HZ)); //to free buffer
#endif
...
#endif
#if defined(_DPMAIF_MED_SUPPORT_)
	__pm_stay_awake(transmit_wakelock);
#endif
...
#if defined(CONFIG_HW_NAT) && defined(_DPMAIF_MED_SUPPORT_)
	...
	struct sk_buff *skb, int skb_from_pool, int blocking)
{
	...
	if (qno == 0 || qno == 1) {
#else
	if (qno == 0 || qno == 1 || qno == 5) {
#endif
		CCCI_ERROR_LOG(dpmaif_ctrl->md_id, TAG,
			"txq(%d) > %d / should not appear txSendSkb\n",
			qno, DPMAIF_TXQ_NUM);
		return 0;
	}

	__pm_wakeup_event(transmit_wakelock_timer, jiffies_to_msecs(HZ)); //make data transmit to MD
#endif
...
retry:
...
#if defined(CONFIG_HW_NAT) && defined(_DPMAIF_MED_SUPPORT_)
	if (ppe_hook_tx_modem) {
#ifdef _DPMAIF_GEN98_CODA_V2
		if (eth_pdu_info.eth_mode && ccci_h.data[0] == eth_pdu_info.eth_id)
			ppe_hook_tx_modem(skb, 0, ccci_h.data[0] | 0x80);
		else
#endif
			ppe_hook_tx_modem(skb, 0, ccci_h.data[0]);
	}
#endif
	/* 3 send data. */
	...
	if (eth_pdu_info.eth_mode && ccci_h.data[0] == eth_pdu_info.eth_id)
		set_drb_msg(txq->index, cur_idx, skb->len, prio_count,
				(ccci_h.data[0] | 0x80), skb->protocol);
	else
#endif
		set_drb_msg(txq->index, cur_idx, skb->len, prio_count,
...
static void mtk_ccci_affinity_rta(u32 irq_cpus, u32 push_cpus, int cpu_nr)
{
...
#else
	rxq->pit_size_cnt = (gMedmcuPitNatSize /sizeof(struct dpmaifq_normal_pit));
#ifdef DPMAIF_DEBUG_LOG
	CCCI_HISTORY_LOG(-1, TAG, "pit(nat)_size_cnt:%d, PitNatSize:%lld, entrySize:%d\n",
		rxq->pit_size_cnt, gMedmcuPitNatSize, (int)sizeof(struct dpmaifq_normal_pit));
	CCCI_NORMAL_LOG(-1, TAG, "pit(nat)_size_cnt:%d, PitNatSize:%lld, entrySize:%d\n",
		rxq->pit_size_cnt, gMedmcuPitNatSize, (int)sizeof(struct dpmaifq_normal_pit));
#endif
#endif
#ifndef _DPMAIF_MED_SUPPORT_
...
#else
	/*MED enabled case*/
	rxq->pit_base = medmcu_pit_nat_base_virt;	/*PIT_NAT base address*/
	if (!rxq->pit_base) {
		CCCI_ERROR_LOG(-1, TAG, "assign PIT_NAT memory fail\r\n");
		return -1;
	}
	rxq->pit_phy_addr = gMedmcuPitNatPhyBase;
#endif
	if (rxq->pit_base == NULL) {
		CCCI_ERROR_LOG(-1, TAG, "pit request fail\n");
		return LOW_MEMORY_PIT;
...
#else
	CCCI_NORMAL_LOG(-1, TAG, "dpmaif_rx_buf_init: PitNat: phy:0x%llx virt:0x%llx size:0x%x, virt(long):0x%llx\n",
		(uint64_t) gMedmcuPitNatPhyBase, (uint64_t) (phys_addr_t) rxq->pit_base,
		(uint32_t) gMedmcuPitNatSize, (unsigned long long)rxq->pit_base);
	CCCI_HISTORY_TAG_LOG(-1, TAG, "dpmaif_rx_buf_init: PitNat: phy:0x%llx virt:0x%llx size:0x%x, virt(long):0x%llx\n",
		(uint64_t) gMedmcuPitNatPhyBase, (uint64_t) (phys_addr_t) rxq->pit_base,
		(uint32_t) gMedmcuPitNatSize, (unsigned long long)rxq->pit_base);
...
#if defined(_DPMAIF_MED_SUPPORT_)
		CCCI_NORMAL_LOG(-1, TAG,
				"printf bat_req.bat_phy_addr: 0x%llx\n",
				rxq->bat_req.bat_phy_addr);
#endif
...
#else
		drv_dpmaif_dl_set_pit_size(rxq->index, (gMedmcuPitSize / sizeof(struct dpmaifq_normal_pit)));
#endif
...
#ifdef _DPMAIF_GEN98_CODA_V2
phys_addr_t gMedmcuDrb5PhyBase;
unsigned long long gMedmcuDrb5Size;
#endif
#endif
static int dpmaif_tx_buf_init(struct dpmaif_tx_queue *txq)
{
...
#ifdef _DPMAIF_MED_SUPPORT_
	if (txq->index == 0) {//MED Q0
		txq->drb_size_cnt = (gMedmcuDrb0Size /sizeof(struct dpmaif_drb_pd));
		CCCI_HISTORY_LOG(-1, TAG, "MED drb0 size cnt:%d\n", txq->drb_size_cnt);
	} else if (txq->index == 1){//MED Q1
		txq->drb_size_cnt = (gMedmcuDrb1Size /sizeof(struct dpmaif_drb_pd));
		CCCI_HISTORY_LOG(-1, TAG, "MED drb1 size cnt:%d\n", txq->drb_size_cnt);
#ifndef _DPMAIF_GEN98_CODA_V2
	}
#else
	} else if (txq->index == 5) {//MED Q5
		txq->drb_size_cnt = (gMedmcuDrb5Size /sizeof(struct dpmaif_drb_pd));
		CCCI_HISTORY_LOG(-1, TAG, "MED drb1 size cnt:%d\n", txq->drb_size_cnt);
	}
#endif
#endif
	/* alloc buffer for HW && AP SW */
#ifdef _DPMAIF_MED_SUPPORT_
if (txq->index != 0 && txq->index != 1 && txq->index != 5) {
#endif
	...
	if (txq->index == 0) {//MED Q0
		txq->drb_base = &dummyDrbBase0; //to-do: review dummy virt_base is ok
		txq->drb_phy_addr = gMedmcuDrb0PhyBase;
	} else if (txq->index == 1){//MED Q1
		txq->drb_base = &dummyDrbBase1; //to-do: review dummy virt_base is ok
		txq->drb_phy_addr = gMedmcuDrb1PhyBase;
#ifndef _DPMAIF_GEN98_CODA_V2
	}
#else
	} else if (txq->index == 5){//MED Q5
		txq->drb_base = &dummyDrbBase5; //to-do: review dummy virt_base is ok
		txq->drb_phy_addr = gMedmcuDrb5PhyBase;
	}
#endif
}
#endif
...
#ifdef DPMAIF_DEBUG_LOG
	CCCI_NORMAL_LOG(-1, TAG,
		"drb(%d) base_addr: virt = 0x%p, phy = 0x%llx\n",
		txq->index, p_ul_que->drb_base, (unsigned long long)base_addr);
#endif
 ...
 * ========================================================
 */
static void ccci_hif_dpmaif_set_clk(unsigned int on)
{
	int ret = 0;

	if (IS_ERR(dpmaif_ctrl->clk_ref0))
		return;
	else if (on) {
...
static void dpmaif_set_addr_and_size_from_medmcu(void)
{
	struct medmcu_desc_info_t *desc = get_desc_info();

	gMedmcuPitNatPhyBase = desc[PIT_NAT].phy_base;
	gMedmcuPitNatSize = desc[PIT_NAT].sz;
	medmcu_pit_nat_base_virt = desc[PIT_NAT].base_virt;

	gMedmcuPitPhyBase = desc[PIT].phy_base;
	gMedmcuPitSize = desc[PIT].sz;

	gMedmcuDrb0PhyBase = desc[DRB0].phy_base;
	gMedmcuDrb1PhyBase = desc[DRB1].phy_base;
	gMedmcuDrb0Size = desc[DRB0].sz;
	gMedmcuDrb1Size = desc[DRB1].sz;

#if defined(_DPMAIF_GEN98_CODA_V2)
	gMedmcuDrb5PhyBase = desc[DRB2].phy_base;
	gMedmcuDrb5Size = desc[DRB2].sz;
#endif
}
#endif
...
int dpmaif_start(unsigned char hif_id)
{
...
#if defined(_DPMAIF_MED_SUPPORT_)
		dpmaif_set_addr_and_size_from_medmcu();
#endif
...
#else
		ret = dpmaif_alloc_med_rx_buf(&rxq->bat_req, i,
				rxq->bat_req.bat_size_cnt, 0, 1);
#endif
		if (ret) {
...
#else
		ret = dpmaif_alloc_med_rx_frag(&rxq->bat_frag, i,
				rxq->bat_frag.bat_size_cnt, 0, 1);
#endif
		if (ret) {
			CCCI_HISTORY_LOG(-1, TAG,
...
#ifdef BAT_CNT_BURST_UPDATE
#ifdef HW_FRG_FEATURE_ENABLE
		if (rxq->bat_frag.bat_size_cnt <= 1) {
			//prevent bat_req.bat_size_cnt - 1 overflow
			CCCI_NORMAL_LOG(-1, TAG, "dpmaif:start low fbat size count\n");
			return DATA_CHECK_FAIL;
		}
		ret = drv_dpmaif_dl_add_frg_bat_cnt(i,
			(unsigned short)(rxq->bat_frag.bat_size_cnt - 1));
		if (ret < 0) {
#if IS_ENABLED(CONFIG_MTK_AEE_FEATURE)
			aee_kernel_warning("ccci",
...
#endif
		if (rxq->bat_req.bat_size_cnt <= 1) {
			//prevent bat_req.bat_size_cnt - 1 overflow
			CCCI_NORMAL_LOG(-1, TAG, "dpmaif:start low bat size count\n");
			return DATA_CHECK_FAIL;
		}
		ret = drv_dpmaif_dl_add_bat_cnt(i,
				(unsigned short)(rxq->bat_req.bat_size_cnt - 1));
		if (ret < 0) {
#if IS_ENABLED(CONFIG_MTK_AEE_FEATURE)
			aee_kernel_warning("ccci",
	...
	rxq->page_low_mem_idx = -1;
#ifndef DPMAIF_MEDHW_RXIRQ
	ipi_ret = mtk_ipi_register(&scp_ipidev, IPI_IN_MED_CCCI,
		(void *)dpmaif_ipi_rx_request, NULL, msg_med_ccci);
	CCCI_HISTORY_TAG_LOG(-1, TAG, "dpmaif:start, register ipi:%d\n", ipi_ret);
#endif
	...
	CCCI_NORMAL_LOG(-1, TAG, "dpmaif:start end: %d\n", ret);
#endif
...
#if defined(_DPMAIF_GEN98_CODA_V2)
static void dpmaif_hw_reset(unsigned char md_id)
{
...
int dpmaif_stop(unsigned char hif_id)
{
	if (dpmaif_ctrl->dpmaif_state == HIFDPMAIF_STATE_PWROFF
		...
		return 0;
	CCCI_NORMAL_LOG(-1, TAG, "dpmaif:stop\n");
#ifdef DPMAIF_DEBUG_LOG
	CCCI_HISTORY_LOG(-1, TAG, "dpmaif:stop\n");
#else
	CCCI_DEBUG_LOG(-1, TAG, "dpmaif:stop\n");
	CCCI_HISTORY_TAG_LOG(-1, TAG, "dpmaif:stop\n");
#endif
	dpmaif_disable_irq(dpmaif_ctrl);
	...
	/* Need to Wait Medmcu ack before reset hw*/
#ifdef _DPMAIF_MED_SUPPORT_
	/*a. send notification to Medmcu for stoping accessing hw*/
	atomic_set(&med_mcu_ack_flag, 0);
	ccci_dpmaif_notify(CCCI_DPMAIF_EVENT_STOP);
	CCCI_NORMAL_LOG(-1, TAG, "dpmaif:waiting medmcu ack\n");
	...
	CCCI_HISTORY_TAG_LOG(-1, TAG, "dpmaif:medmcu acked\n");

	/*c. after receive ack, continue the following reset hw logic*/
#endif
...
static int dpmaif_resume(unsigned char hif_id)
{
	...
	if (drv_dpmaif_check_power_down() == false) {
		...
		CCCI_NORMAL_LOG(0, TAG, "sys_resume no need restore\n");
	} else {
		/*IP power down before and need to restore*/
		CCCI_NORMAL_LOG(0, TAG,
			"sys_resume power down before(0x%x, 0x%x, 0x%x)\n",
			drv_dpmaif_ul_get_rwidx(0),
			drv_dpmaif_ul_get_rwidx(1),
			drv_dpmaif_ul_get_rwidx(3));
#if !defined(_DPMAIF_GEN98_CODA_)
...
unsigned int backup_L2RIMR0 = 0;
unsigned int backup_L2TIMR0 = 0;
static void dpmaif_sysresume(void)
{
	CCCI_NORMAL_LOG(-1, TAG,
			"dpmaif_resume backup_L2RIMR0:0x%x, backup_L2TIMR0:0x%x\n", backup_L2RIMR0, backup_L2TIMR0);
	DPMA_WRITE_AO_UL(NRL2_DPMAIF_AO_UL_APDL_L2TIMSR0, backup_L2RIMR0); //10011098
	DPMA_WRITE_AO_UL(NRL2_DPMAIF_AO_UL_AP_L2TIMSR0, backup_L2TIMR0); //10011088
	mb();
	CCCI_NORMAL_LOG(-1, TAG,
			"dpmaif_resume L2RIMR0:0x%x, L2TIMR0:0x%x\n",
			DPMA_READ_AO_UL(NRL2_DPMAIF_AO_UL_APDL_L2TIMR0), DPMA_READ_AO_UL(NRL2_DPMAIF_AO_UL_AP_L2TIMR0));
#if !defined(_DPMAIF_GEN98_CODA_)
	if (dpmaif_ctrl->dpmaif_state != HIFDPMAIF_STATE_PWRON &&
...
{
	backup_L2RIMR0 = DPMA_READ_AO_UL(NRL2_DPMAIF_AO_UL_APDL_L2TIMR0); //10011090
	backup_L2TIMR0 = DPMA_READ_AO_UL(NRL2_DPMAIF_AO_UL_AP_L2TIMR0);  //10011080
	CCCI_NORMAL_LOG(-1, TAG,
			"dpmaif_suspend L2RIMR0:0x%x, L2TIMR0:0x%x\n", backup_L2RIMR0, backup_L2TIMR0);
...
#if defined(_DPMAIF_GEN98_CODA_V2) && defined(_DPMAIF_MED_SUPPORT_)
#ifdef DPMAIF_MEDHW_RXIRQ
struct platform_device *tmp_pdev = NULL;
static irqreturn_t med_rx_isr(int irq, void *data)
{
	atomic_inc(&med_rx_irq_count);
	//CCCI_ERROR_LOG(-1, TAG, "[MEDMCU] %s irq received\n", __func__);
	DPMA_WRITE_MED_INT(MEDHW_INT_NOTIF_W1C, (1 << 26));
	//CCCI_ERROR_LOG(-1, TAG, "[MEDMCU] %s W1C\n", __func__);
	dpmaif_ipi_rx_request(0, NULL, 0);
	return IRQ_HANDLED;
}
unsigned int test_irq_no = 0;
#endif
#endif
...
int ccci_dpmaif_hif_init(struct device *dev)
{
...
#if defined(_DPMAIF_GEN98_CODA_V2)
	hif_ctrl->clk_ref1 = devm_clk_get(dev, "infra-dpmaif-rg-mmw-clk");
	if (IS_ERR(hif_ctrl->clk_ref1)) {
		CCCI_ERROR_LOG(md_id, TAG,
			 "dpmaif get infra-dpmaif-rg-mmw-clk failed\n");
...
#if defined(_DPMAIF_GEN98_CODA_)
	hif_ctrl->dpmaif_ao_dl_sram_base = hif_ctrl->dpmaif_pd_ul_base + 0xC00;
	hif_ctrl->dpmaif_ao_ul_sram_base = hif_ctrl->dpmaif_pd_ul_base + 0xD00;
#endif
...
#ifdef _DPMAIF_MED_SUPPORT_
	/*get med base address*/
	hif_ctrl->dpmaif_naddr_medhw_bmp_base = of_iomap(node, 4);
	CCCI_NORMAL_LOG(md_id, TAG,
			"medhw_bmp_base:0x%px\n",
			(void *)hif_ctrl->dpmaif_naddr_medhw_bmp_base);
	if (hif_ctrl->dpmaif_naddr_medhw_bmp_base == 0) {
		CCCI_ERROR_LOG(md_id, TAG,
			"ap_dpmaif error: medhw_bmp_base=0x%p, iomap fail\n",
			(void *)hif_ctrl->dpmaif_naddr_medhw_bmp_base);
		ret = -1;
		goto DPMAIF_INIT_FAIL;
	}
	hif_ctrl->dpmaif_naddr_medhw_ssr1_base = of_iomap(node, 5);
	CCCI_NORMAL_LOG(md_id, TAG,
			"medhw_ssr1_base:0x%px\n",
			(void *)hif_ctrl->dpmaif_naddr_medhw_ssr1_base);
	if (hif_ctrl->dpmaif_naddr_medhw_ssr1_base == 0) {
		CCCI_ERROR_LOG(md_id, TAG,
			"ap_dpmaif error: medhw_ssr1_base=0x%p, iomap fail\n",
			(void *)hif_ctrl->dpmaif_naddr_medhw_ssr1_base);
		ret = -1;
		goto DPMAIF_INIT_FAIL;
	}
#ifdef _DPMAIF_GEN98_CODA_V2
	hif_ctrl->dpmaif_naddr_medhw_int_mng_base = of_iomap(node, 6);
	CCCI_NORMAL_LOG(md_id, TAG,
			"medhw_int_base:0x%px\n",
			(void *)hif_ctrl->dpmaif_naddr_medhw_int_mng_base);
	if (hif_ctrl->dpmaif_naddr_medhw_int_mng_base == 0) {
		CCCI_ERROR_LOG(md_id, TAG,
			"ap_dpmaif error: medhw_int_base=0x%p, iomap fail\n",
			(void *)hif_ctrl->dpmaif_naddr_medhw_int_mng_base);
		ret = -1;
		goto DPMAIF_INIT_FAIL;
	}
...
#if defined(_DPMAIF_GEN98_CODA_V2) && defined(_DPMAIF_MED_SUPPORT_)
	/*kc test*/
	test_irq_no = irq_of_parse_and_map(node, 1);
#ifdef DPMAIF_MEDHW_RXIRQ
	ret = request_irq(test_irq_no, med_rx_isr,
		IRQF_TRIGGER_NONE, "MED_RX_IRQ",
		NULL);
	CCCI_NORMAL_LOG(-1, TAG, "[MEDMCU]test_irq_no ret = %d, test_irq_no:%d\n", ret, test_irq_no);
#endif
	/*kc test*/
#endif
	...
	#if defined(_DPMAIF_MED_SUPPORT_) && defined(CONFIG_MTK_TINYSYS_MEDMCU_SUPPORT)
	/*register MEDMCU notify*/
	CCCI_NORMAL_LOG(-1, TAG, "dpmaif: scp_A_register_notify\n");
	scp_A_register_notify(&apsync_notifier);

	dpmaif_ctrl->rx_wakelock = wakeup_source_register(NULL, "ccci_data");
	if (!dpmaif_ctrl->rx_wakelock) {
		CCCI_ERROR_LOG(md_id, TAG,
	...
	push_wakelock = wakeup_source_register(NULL, "ccci_data_push");
	if (!push_wakelock) {
		CCCI_ERROR_LOG(md_id, TAG,
			"%s %d: init wakeup source fail",
			__func__, __LINE__);
	...
	transmit_wakelock = wakeup_source_register(NULL, "ccci_data_transmit");
	if (!transmit_wakelock) {
		CCCI_ERROR_LOG(md_id, TAG,
			"%s %d: init wakeup source fail",
			__func__, __LINE__);
		ret = -1;
	...
	transmit_wakelock_timer = wakeup_source_register(NULL, "ccci_data_transmit_timer");
	if (!transmit_wakelock_timer) {
		CCCI_ERROR_LOG(md_id, TAG,
			"%s %d: init wakeup source fail",
			__func__, __LINE__);
		ret = -1;
...
#else
	CCCI_ERROR_LOG(-1, TAG, "dpmaif not support");
#endif
	return 0;
}
