...
static unsigned int port_char_dev_poll(struct file *fp,
...
static int port_char_recv_skb(struct port_t *port, struct sk_buff *skb)
{
	...
	if (!atomic_read(&port->usage_cnt) &&
		(/* port->rx_ch != CCCI_UART2_RX && */ // change the setting to drop msg before opening
#if defined(CONFIG_MTK_MD3_SUPPORT) && (CONFIG_MTK_MD3_SUPPORT > 0)
		port->rx_ch != CCCI_C2K_AT &&
#endif
...
static void port_char_dump_info(struct port_t *port, unsigned int flag)
{
	if (port == NULL) {
