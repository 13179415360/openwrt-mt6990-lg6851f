...
#include "ccci_fsm.h"
#include "port_smem.h"
#include <linux/hrtimer.h> /* LG6851F Stage4I */
#include <linux/sched/clock.h> /* LG6851F Stage4I: local_clock */
#include <linux/mm.h> /* LG6851F Stage4I: vm_flags_set */
...
static void collect_ccb_info(int md_id, struct ccci_smem_port *smem_port)
{
	...
	for (i = SMEM_USER_CCB_START; i <= SMEM_USER_CCB_END; i++) {
		if (smem_port->user_id == i) {
			...
			hrtimer_setup(&smem_port->notify_timer, smem_tx_timer_func, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
			/* LG6851F Stage4I v5: callback supplied by hrtimer_setup(). */
			break;
		}
	}
...
static int port_smem_tx_nofity(struct port_t *port, unsigned int user_data)
{
	struct ccci_smem_port *smem_port =
...
static int port_smem_rx_poll(struct port_t *port, unsigned int user_data)
{
	struct ccci_smem_port *smem_port =
		(struct ccci_smem_port *)port->private_data;
#ifdef DEBUG_FOR_CCB
...
static int port_smem_rx_wakeup(struct port_t *port)
{
	struct ccci_smem_port *smem_port =
		(struct ccci_smem_port *)port->private_data;
	unsigned long flags;
...
static long port_ccb_ioctl(struct port_t *port, unsigned int cmd,
			   unsigned long arg)
{
	int md_id = port->md_id;
	long ret = 0;
...
static long port_smem_ioctl(struct port_t *port, unsigned int cmd,
			    unsigned long arg)
{
	long ret = 0;
	int md_id = port->md_id;
	...
	struct ccci_smem_port *smem_port =
		(struct ccci_smem_port *)port->private_data;
	unsigned char *ptr;
	struct ccci_ccb_debug debug_in = {0};
	struct ccci_ccb_debug debug_out = {0};
...
static int port_smem_init(struct port_t *port)
{
