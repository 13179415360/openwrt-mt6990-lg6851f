...
static int port_ipc_recv_match(struct port_t *port, struct sk_buff *skb)
{
...
static long port_ipc_ioctl(struct file *file, unsigned int cmd,
	unsigned long arg)
{
	long ret = 0;
...
static void port_ipc_md_state_notify(struct port_t *port, unsigned int state)
{
	struct ccci_ipc_ctrl *ipc_ctrl =
		(struct ccci_ipc_ctrl *)port->private_data;
...
static unsigned int port_ipc_poll(struct file *fp,
...
static int port_ipc_init(struct port_t *port)
{
