...
#include "ccci_swtp.h"
#include "ccci_fsm.h"
#include <linux/gpio/consumer.h> /* LG6851F Stage5J: descriptor debounce */
#include <linux/errno.h> /* LG6851F Stage5J: debounce fallback errno */

/* LG6851F Stage5J: bridge legacy numeric GPIO debounce to gpiod API. */
static int lg6851f_gpio_set_debounce(unsigned int gpio,
...
static int switch_MD1_Tx_Power(unsigned int mode)
{
...
static int swtp_md_tx_power_req_hdlr(int md_id, int data)
{
	int ret = 0;
...
int swtp_init(int md_id)
{
	...
	if (node) {
		...
		if (ret)
			...
			swtp_data[md_id].eint_type[i] = ints1[1];
#endif
			lg6851f_gpio_set_debounce(swtp_data[md_id].gpiopin[i],
