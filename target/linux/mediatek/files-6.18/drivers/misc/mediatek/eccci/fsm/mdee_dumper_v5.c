	...
	char *aed_str, int db_opt)
{
 ...
 err_exit1:
	if ((dump_flag & CCCI_AED_DUMP_CCIF_REG) &&
		(mdss_dbg && mdss_dbg->base_ap_view_vir)) {
	...
	if ((dump_flag & CCCI_AED_DUMP_MD_IMG_MEM) &&
		(mem_layout && mem_layout->md_bank0.base_ap_view_vir)) {
...
static void mdee_info_dump_v5(struct ccci_fsm_ee *mdee)
{
	...
	ex_info = kzalloc(AED_STR_LEN, GFP_ATOMIC);
	if (ex_info == NULL) {
	...
	if (debug_info->par_data_source == MD_EE_DATA_IN_GPD) {
		...
		(mdccci_dbg && mdccci_dbg->base_ap_view_vir) &&
		(mdss_dbg && mdss_dbg->base_ap_view_vir)) {
...
#ifdef _CTRL_PLANE_GEN98_
	} else if ((reg_value[0] == 0x5443000F) ||
				(reg_value[0] == 0) ||
...
static void __maybe_unused mdee_dumper_v5_emimpu_callback(
		unsigned int emi_id,
