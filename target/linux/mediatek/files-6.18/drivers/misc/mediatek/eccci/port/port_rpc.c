...
#include "port_rpc.h"
#include <linux/mm.h> /* LG6851F Stage5J: vm_flags_set */
...
static int get_eint_attr_val(int md_id, struct device_node *node, int index)
{
	...
	if (index < 0 || index >= MD_SIM_MAX) {
		CCCI_ERROR_LOG(md_id, RPC, "%s: invalid index = %d\n", __func__, index);
		return -EINVAL;
	}

	for (type = 0; type >= 0 && type < SIM_HOT_PLUG_EINT_MAX; type++) {
...
static void get_dtsi_eint_node(int md_id)
{
	...
	for (i = 0; i < MD_SIM_MAX; i++) {
		...
		if (node != NULL) {
			...
			CCCI_INIT_LOG(md_id, RPC, "%s: node %d no found in uppercase\n",
				eint_node_prop.name[i].node_name, i);
		}

		len_temp = strlen(eint_node_prop.name[i].node_name);
		temp_qstring = kzalloc((len_temp+1), GFP_KERNEL);
		if (temp_qstring != NULL) {
			for (s = 0; s < len_temp; s++) {
				temp_qstring[s] = tolower((unsigned char)eint_node_prop.name[i].node_name[s]);
			}
			node = of_find_node_by_name(NULL, temp_qstring);
			if (node != NULL) {
				CCCI_INIT_LOG(md_id, RPC, "%s: node %d found in lowercase\n",
						temp_qstring, i);
				eint_node_prop.ExistFlag |= (1U << i);
				get_eint_attr_val(md_id, node, i);
			} else {
				CCCI_INIT_LOG(md_id, RPC, "%s: node %d no found in lowercase\n",
						temp_qstring, i);
			}
		} else {
			CCCI_INIT_LOG(md_id, RPC, "Fail alloc Mem for temp_qstring\n");
		}
		kfree(temp_qstring);
	}
}

static int get_eint_attr_DTSVal(int md_id, const char *name,
...
/*
 * Quectel's production kernel overrides the LK/DTS DRDI value with item 5
 * from the per-device qraw partition.  LG6851F stores that 24-byte item at
 * qraw+0xc9: a 20-byte "QLDRDISETIDX" magic field followed by an unaligned
 * little-endian u32 set index.  Keep the lookup board-specific and preserve
 * the stock set0 fallback while devtmpfs/MMC is not ready during early boot.
 */
#define LG6851F_QRAW_DRDI_OFFSET	0xc9
#define LG6851F_DRDI_MAGIC	"QLDRDISETIDX"

struct lg6851f_qraw_drdi {
	u8 magic[20];
	u8 setidx[sizeof(u32)];
} __packed;

static u32 lg6851f_qraw_get_drdi_setidx(void)
{
	static const char * const paths[] = {
		"/dev/block/qraw",
		"/dev/mmcblk0p42",
	};
	struct lg6851f_qraw_drdi item;
	struct file *filp;
	loff_t pos;
	ssize_t len;
	int i;

	if (!of_machine_is_compatible("fiberhome,lg6851f"))
		return 0;

	for (i = 0; i < ARRAY_SIZE(paths); i++) {
		filp = filp_open(paths[i], O_RDONLY, 0);
		if (IS_ERR(filp))
			continue;

		pos = LG6851F_QRAW_DRDI_OFFSET;
		len = kernel_read(filp, &item, sizeof(item), &pos);
		filp_close(filp, NULL);
		if (len != sizeof(item))
			continue;

		if (memcmp(item.magic, LG6851F_DRDI_MAGIC,
			   strlen(LG6851F_DRDI_MAGIC))) {
			CCCI_ERROR_LOG(-1, RPC,
				"[quectel_drdi] qraw magic mismatch; use set0\n");
			return 0;
		}

		return get_unaligned_le32(item.setidx);
	}
	...
	struct ccci_rpc_md_dtsi_output *output)
{
	...
	case RPC_REQ_PROP_VALUE:
		if (of_machine_is_compatible("fiberhome,lg6851f") &&
		    !strcmp(input->strName, "mediatek,md_drdi_rf_set_idx")) {
			output->retValue = lg6851f_qraw_get_drdi_setidx();
			CCCI_NORMAL_LOG(-1, RPC,
				"[quectel_drdi] qraw return value=0x%x\n",
				output->retValue);
			break;
		}
	...
	struct rpc_buffer *p_rpc_buf, unsigned int tmp_data[])
{
	...
	CCCI_DEBUG_LOG(md_id, RPC, "%s++ %d op_id=0x%X\n", __func__,
		p_rpc_buf->para_num, p_rpc_buf->op_id);
	tmp_data[0] = FS_NO_ERROR;
#ifdef CONFIG_MTK_ECCCI_FLASHLESS
	if (p_rpc_buf->op_id != IPC_RPC_GET_EINT_ATTR_OP &&
			p_rpc_buf->op_id != IPC_RPC_GET_GPIO_ADC_OP &&
			p_rpc_buf->op_id != IPC_RPC_USIM2NFC_OP &&
			p_rpc_buf->op_id != IPC_RPC_DTSI_QUERY_OP) {
		CCCI_NORMAL_LOG(md_id, RPC, "[Error]Unexpected op_id=0x%08X\n",
				p_rpc_buf->op_id);
		tmp_data[0] = FS_NO_ERROR;
		pkt_num = 0;
...
//TODO: Remove when new design with runtime data applies
#if defined(CONFIG_MTK_ECCCI_FLASHLESS)
	case IPC_RPC_SAR_TABLE_IDX_QUERY_OP:
		CCCI_NORMAL_LOG(md_id, RPC,
		"IPC_RPC_SAR_TABLE_IDX_QUERY_OP customerization not support, return 0 directly\n");
		tmp_data[0] = FS_NO_ERROR;
		pkt_num = 0;
		pkt[pkt_num].len = sizeof(int);
		pkt[pkt_num++].buf = (void *)&tmp_data[0];  //RetVal
		pkt[pkt_num].len = sizeof(int);
		pkt[pkt_num++].buf = (void *)&tmp_data[0];  //sar_tbl_idx
		break;
#endif
	default:
		CCCI_NORMAL_LOG(md_id, RPC,
		"[Error]Unknown op_id=0x%08X\n",
		p_rpc_buf->op_id);
		tmp_data[0] = FS_NO_OP;
		pkt_num = 0;
		pkt[pkt_num].len = sizeof(int);
...
/* LG6851F Stage5J: keep conditional exit label referenced. */
if (0)
	goto exit;
exit:
...
static const struct file_operations rpc_dev_fops = {
	...
	.mmap = &port_rpc_dev_mmap,  /*use internal API for AMMS_v3*/
};
...
static int port_rpc_recv_match(struct port_t *port, struct sk_buff *skb)
{
	int md_id = port->md_id;
...
//TODO: Remove when new design with runtime data applies
		...
		case IPC_RPC_SAR_TABLE_IDX_QUERY_OP:
#endif
		case IPC_RPC_AMMS_DRDI_CONTROL:  // AMMS_v3
			is_userspace_msg = 1;
			break;
		default:
			is_userspace_msg = 0;
