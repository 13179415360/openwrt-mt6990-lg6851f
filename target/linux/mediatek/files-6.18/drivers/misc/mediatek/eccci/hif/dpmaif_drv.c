...
#if defined(_E1_SB_SW_WORKAROUND_) || defined(_HW_REORDER_SW_WORKAROUND_)
...
static unsigned char med_src_dest_val[][2] = {{1,0}, {0,1}, {2,1}, {0,2}};
static int med_dl_sw_map_cmd(unsigned char cmd_id, bool frag, unsigned char flush, unsigned int map_idx, unsigned int len)
{
	unsigned int cmd, tbl_size,sub_len;
	unsigned char frag_val;
	int count = 0;

	frag_val = (frag)? 1:0;

	tbl_size = DPMAIF_DL_BAT_ENTRY_SIZE;

	while(len){
		sub_len = len<=0x3ff? len: 0x3ff;
		cmd = (map_idx&0xffff) | (sub_len<<16) | (frag_val<<27) | ((med_src_dest_val[cmd_id][1]&0x3)<<28) | (1<<31);

		DPMA_WRITE_MED_BMP(MEDHW_BMP_FIFO2_SW_PUSH, cmd);

		//POLL_STATUS(DPMA_READ_MED(MEDHW_BMP_FIFO2_SW_PUSH)&(1<<31));
		while (1) {
			if ((DPMA_READ_MED_BMP(MEDHW_BMP_FIFO2_SW_PUSH)&(1<<31)) == 0) {
				break;
			}
			if (++count >= 1600000) {
				CCCI_ERROR_LOG(0, TAG,
					"med_dl_sw_map_cmd fail,%d, %u, %u\n", frag, map_idx, len);
				dpmaif_ctrl->ops->dump_status(
					DPMAIF_HIF_ID, DUMP_FLAG_REG, NULL, -1);
				return HW_REG_TIME_OUT;
			}
		...
		map_idx = (map_idx+sub_len)%tbl_size;
		len = len-sub_len;
	}
	return 0;
}

unsigned int drv_dpmaif_dl_get_med_wridx(unsigned char q_num)
{
	unsigned int widx;

	widx = (DPMA_READ_MED_SSR1(MEDHW_SSR1_DST_RB1_WIDX) & 0x3ffff);

	return widx;
}

int drv_dpamif_med_bat_recycle_cmd(bool frag, unsigned int buf_idx, unsigned int len)
{
	return med_dl_sw_map_cmd(2, frag, 0, buf_idx, len);
}

unsigned int drv_dpamif_med_dl_pit_nat_release(unsigned int pit_num)
{
	unsigned int cmd;
	int count = 0;
	int ret = 0;

	cmd = (pit_num&0x3ffff) | (1<<31);
	DPMA_WRITE_MED_SSR1(MEDHW_SSR1_DST_RB1_DEC, cmd);

	//POLL_STATUS(DPMA_READ_MED(MEDHW_SSR1_DST_RB1_DEC)&(1<<31));
	while (1) {
		if ((DPMA_READ_MED_SSR1(MEDHW_SSR1_DST_RB1_DEC)&(1<<31)) == 0) {
			break;
		}
		if (++count >= 1600000) {
			CCCI_ERROR_LOG(0, TAG,
				"med_dl_pit_nat_release fail\n");
			dpmaif_ctrl->ops->dump_status(
				DPMAIF_HIF_ID, DUMP_FLAG_REG, NULL, -1);
			return HW_REG_TIME_OUT;
		}
...
unsigned int drv_dpmaif_ul_get_ridx(unsigned char q_num)
{
	...
	if (q_num == 5) {
		ridx = (DPMA_READ_AO_UL_SRAM(NRL2_DPMAIF_AO_UL_CH5_STA) >> 16) & 0x0000ffff;
	} 
	else 
#endif
...
#if defined (_DPMAIF_GEN98_CODA_V2)
	if (q_num == 5) {
		return drv_dpmaif_ulq5_add_wcnt(drb_wcnt);
	}
	else if (q_num == 4) {
		return drv_dpmaif_ulq4_add_wcnt(drb_wcnt);
	} else
#endif
	{
...
#if defined (_DPMAIF_GEN98_CODA_V2) 
int drv_dpmaif_ulq5_add_wcnt(unsigned short drb_wcnt)
{
	unsigned int ul_update;
	int count = 0;
	...
	while (1) {
		if ((DPMA_READ_PD_UL(NRL2_DPMAIF_UL_ADD_DESC_CH5) &
			DPMAIF_UL_ADD_NOT_READY) == 0) {
			DPMA_WRITE_PD_UL(NRL2_DPMAIF_UL_ADD_DESC_CH5,
				ul_update);
			break;
		}
		if (++count >= 1600000) {
			CCCI_ERROR_LOG(0, TAG, "drb5_add rdy poll fail: 0x%x\n",
				DPMA_READ_PD_UL(DPMAIF_PD_UL_DBG_STA2));
			dpmaif_ctrl->ops->dump_status(
				DPMAIF_HIF_ID, DUMP_FLAG_REG, NULL, -1);
			return HW_REG_CHK_FAIL;
	...
	count = 0;
	while ((DPMA_READ_PD_UL(NRL2_DPMAIF_UL_ADD_DESC_CH5) &
		DPMAIF_UL_ADD_NOT_READY) == DPMAIF_UL_ADD_NOT_READY) {
		if (++count >= 1600000) {
			CCCI_ERROR_LOG(0, TAG, "drb5_add fail: 0x%x\n",
				DPMA_READ_PD_UL(DPMAIF_PD_UL_DBG_STA2));
			break;
		}
	}
...
int drv_dpmaif_ulq4_add_wcnt(unsigned short drb_wcnt)
{
	unsigned int ul_update;
	int count = 0;
	...
	while (1) {
		if ((DPMA_READ_PD_UL(DPMAIF_PD_UL_ADD_DESC_CH4) &
			DPMAIF_UL_ADD_NOT_READY) == 0) {
			DPMA_WRITE_PD_UL(DPMAIF_PD_UL_ADD_DESC_CH4,
				ul_update);
			break;
		}
		if (++count >= 1600000) {
			CCCI_ERROR_LOG(0, TAG, "drb4_add rdy poll fail: 0x%x\n",
				DPMA_READ_PD_UL(DPMAIF_PD_UL_DBG_STA2));
			dpmaif_ctrl->ops->dump_status(
				DPMAIF_HIF_ID, DUMP_FLAG_REG, NULL, -1);
			return HW_REG_CHK_FAIL;
	...
	count = 0;
	while ((DPMA_READ_PD_UL(DPMAIF_PD_UL_ADD_DESC_CH4) &
		DPMAIF_UL_ADD_NOT_READY) == DPMAIF_UL_ADD_NOT_READY) {
		if (++count >= 1600000) {
			CCCI_ERROR_LOG(0, TAG, "drb4_add fail: 0x%x\n",
				DPMA_READ_PD_UL(DPMAIF_PD_UL_DBG_STA2));
			break;
		}
	}
...
#if defined(_DPMAIF_GEN98_CODA_)
void drv_dpmaif_sram_init(void)
{
	unsigned int value;
...
void drv_dpmaif_hw_init_done(void)
{
...
void drv_dpmaif_ul_update_drb_size(unsigned char q_num, unsigned int size)
{
	...
	if (q_num==5) {
		...
		set_size = size/4;
		old_size = DPMA_READ_AO_UL_SRAM(NRL2_DPMAIF_UL_CHNL5_CON1);

		old_size &= ~DPMAIF_DRB_SIZE_MSK;
		old_size |= (set_size & DPMAIF_DRB_SIZE_MSK);

		DPMA_WRITE_AO_UL_SRAM(NRL2_DPMAIF_UL_CHNL5_CON1, old_size);
	} else {
#endif
		/* 1. bit 15~0: DRB count, in word(4 bytes) curr: 512*8/4 */
		set_size = size/4;
	...
	unsigned int lb_addr, unsigned int hb_addr)
{
	...
	if (q_num==5) {
		...
		DPMA_WRITE_AO_UL_SRAM(NRL2_DPMAIF_UL_CHNL5_CON0, lb_addr);

		old_addr = DPMA_READ_AO_UL_SRAM(NRL2_DPMAIF_UL_CHNL5_CON1);

		old_addr &= ~DPMAIF_DRB_ADDRH_MSK;
		old_addr |= ((hb_addr<<24) & DPMAIF_DRB_ADDRH_MSK);
		/* 2. bit 31~24: drb base addr high 8bits, curr: hb_addr */
		DPMA_WRITE_AO_UL_SRAM(NRL2_DPMAIF_UL_CHNL5_CON1, old_addr);
	} else {
#endif
		/* 2 bit 31~0: drb base addr low 32bits, curr: lb_addr */
...
#if defined (_DPMAIF_GEN98_CODA_V2)
	if(q_num == 5) {
		ul_rdy_en = DPMA_READ_AO_UL_SRAM(NRL2_DPMAIF_UL_CHNL5_MASK_ARB_EN);
		...
		DPMA_WRITE_AO_UL_SRAM(NRL2_DPMAIF_UL_CHNL5_MASK_ARB_EN, ul_rdy_en);
	} else {
#endif
#if defined(MT6297) || defined(_DPMAIF_GEN98_CODA_)
...
#if defined (_DPMAIF_GEN98_CODA_V2)
	if(q_num == 5) {
		ul_arb_en = DPMA_READ_AO_UL_SRAM(NRL2_DPMAIF_UL_CHNL5_MASK_ARB_EN);
		...
		DPMA_WRITE_AO_UL_SRAM(NRL2_DPMAIF_UL_CHNL5_MASK_ARB_EN, ul_arb_en);
	} else {
#endif
#if defined(MT6297) || defined(_DPMAIF_GEN98_CODA_)
...
#if defined(_DPMAIF_MED_SUPPORT_)
	/*MED dsiable DL interrupt*/
	sts = DPMA_READ_AO_UL(NRL2_DPMAIF_AO_UL_AP_L1TIMR0);
	sts |= AP_L1TIMR0_DL_TOP_INTM;
	DPMA_WRITE_AO_UL(NRL2_DPMAIF_AO_UL_AP_L1TIMR0,sts);

	/* Set AP IP busy */
	/* 1. clear dummy sts*/
	DPMA_WRITE_PD_MISC(DPMAIF_PD_AP_IP_BUSY, 0xFFFFFFFF);
	/* 2. mask DL/UL busy*/
	DPMA_WRITE_AO_UL(NRL2_DPMAIF_AO_UL_AP_DL_UL_IP_BUSY_MASK, AP_UL_IP_BUSY_MASK);
#else
	/* Set AP IP busy */
	/* 1. clear dummy sts*/
	DPMA_WRITE_PD_MISC(DPMAIF_PD_AP_IP_BUSY, 0xFFFFFFFF);
