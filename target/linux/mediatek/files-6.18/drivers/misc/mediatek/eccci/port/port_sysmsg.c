...
#define MD_RF_NOTIFY(bit, func, name) \
	void func(unsigned int para0, unsigned int para1); \
...
static void exec_ccci_sys_call_back(int md_id, int cb_id, int data)
{
	ccci_sys_cb_func_t func;
	int id;
...
static int thermal_msg_count = 0;  // log reduction
// @}
...
static void sys_msg_handler(struct port_t *port, struct sk_buff *skb)
{
	...
	if (ccci_h->data[1] != MD_RF_MAX_TEMPERATURE_SUB6) {
		...
		if (thermal_msg_count < MAX_THERMAL_MSG_NUM) {
			...
			CCCI_NORMAL_LOG(md_id, SYS, "system message (%x %x %x %x) msg_count:%d\n",
				ccci_h->data[0], ccci_h->data[1],
				ccci_h->channel, ccci_h->reserved, thermal_msg_count);
			thermal_msg_count = 0;
		}
	}
