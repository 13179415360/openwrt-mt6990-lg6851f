...
static int ccmni_send_pkt(int md_id, int ccmni_idx, void *data, int is_ack)
{
	...
	int ret;
#ifdef PORT_NET_DUMP
	char port_net_dump_param[256] = {0};
#endif
...
#ifdef PORT_NET_DUMP
	port_net_dump_fill(port, 1, skb, port_net_dump_param, sizeof(port_net_dump_param));
#endif
	ret = port_net_send_skb_to_md(port, is_ack, skb);
#ifdef PORT_NET_DUMP
	port_net_dump(port_net_dump_param, md_id, ccci_h->seq_num);  // seq_num is filled in port_net_send()
#endif
	CCCI_DEBUG_LOG(md_id, NET,
		"port %s send: %08X, %08X, %08X, %08X, %d\n", port->name,
		ccci_h->data[0], ccci_h->data[1], ccci_h->channel,
		ccci_h->reserved, ccci_h->seq_num);
...
static int ccmni_napi_poll(int md_id, int ccmni_idx,
...
static int port_net_init(struct port_t *port)
{
	...
	if (!per_md_data) {
		CCCI_ERROR_LOG(md_id, NET, "ccci_get_per_md_data fail!\n");
		return -EFAULT;
	}
...
static int port_net_recv_skb(struct port_t *port, struct sk_buff *skb)
{
	...
	if (port->hif_id == MD1_NET_HIF) {
		...
		skb_pull(skb, sizeof(struct ccci_header));
		CCCI_DEBUG_LOG(port->md_id, NET,
			"port %s recv: 0x%08X, 0x%08X, %08X, 0x%08X, %d\n",
			port->name,
			ccci_h->data[0], ccci_h->data[1], ccci_h->channel,
			ccci_h->reserved, ccci_h->seq_num);
	}
...
#ifdef PORT_NET_DUMP
	port_net_dump_fill(port, 0, skb, port_net_dump_param, sizeof(port_net_dump_param));
	port_net_dump(port_net_dump_param, port->md_id, ccci_h->seq_num);
#endif
...
static void port_net_md_dump_info(struct port_t *port, unsigned int flag)
{
	if (port == NULL) {
