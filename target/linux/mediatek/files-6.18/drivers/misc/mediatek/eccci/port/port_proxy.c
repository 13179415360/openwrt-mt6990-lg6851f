...
static int port_dev_kernel_read(struct port_t *port, char *buf, int size)
{
...
READ_START:
	...
	if (size >= read_len) {
		...
		/* no need, the type of sk_buff_head.qlin is __u32
		if (port->rx_skb_list.qlen < 0) {
			spin_unlock_irqrestore(&port->rx_skb_list.lock, flags);
...
// colgin @{
int mtk_ccci_write_data(int index, char *buf, int size){
	return mtk_ccci_send_data(index, buf, size);
}
EXPORT_SYMBOL(mtk_ccci_write_data);

int mtk_ccci_open_port(char *port_name)
{
	...
	port_idx = mtk_ccci_request_port(port_name);
	if (port_idx < 0)
		return -EINVAL;

	ret = find_port_by_channel(port_idx, &port);
	if (ret < 0)
		return -EINVAL;

	if (!(port->flags & PORT_F_CCCI_KERNEL) || atomic_cmpxchg(&port->usage_cnt, 0, 1) != 0) {
	...
	CCCI_NORMAL_LOG(md_id, CHAR,
		"port %s open with flag 0 by %s\n", port->name, current->comm);
	return port_idx;
}
EXPORT_SYMBOL(mtk_ccci_open_port);

int mtk_ccci_close_port(int index)
{
	struct port_t *port = NULL;
	int port_idx = 0;
	struct sk_buff *skb = NULL;
	...
	port_idx = find_port_by_channel(index, &port);
	if (port_idx < 0)
		return -EINVAL;
	if (!(port->flags & PORT_F_CCCI_KERNEL) ||
		atomic_read(&port->usage_cnt) <= 0)
		return -EBUSY;
...
EXPORT_SYMBOL(mtk_ccci_close_port);
// @}
...
int port_dev_open(struct inode *inode, struct file *file)
{
	...
	if (port->rx_ch != CCCI_CCB_CTRL && atomic_cmpxchg(&port->usage_cnt, 0, 1) != 0) {
		CCCI_ERROR_LOG(port->md_id, CHAR,
			"port %s open fail with EBUSY  pid=%d\n", port->name,current->pid);
		/*BEGIN mipc port debug,add by haowei.cheng*/
		CCCI_ERROR_LOG(port->md_id, CHAR,
			"port %s has been opened by %s pid=%d,usage_cnt=%d\n",
			port->name,
			port->current_common,
			port->current_pid,
			atomic_read(&port->usage_cnt));
		/*END mipc port debug,add by haowei.cheng*/
		return -EBUSY;
	}
	md_id = port->md_id;
	CCCI_NORMAL_LOG(md_id, CHAR,
	...
	{
		int len=0;
		port->current_pid = current->pid;
		len = strlen(current->comm);
		if(len >= sizeof(port->current_common)){
			memset(port->current_common,0,sizeof(port->current_common));
			memcpy(port->current_common,current->comm,(sizeof(port->current_common)-2));
		}else{
			memcpy(port->current_common,current->comm,len);
		}
	}
	/*END mipc port debug,add by haowei.cheng*/
...
int port_dev_close(struct inode *inode, struct file *file)
{
	...
	/*BEGIN mipc port debug,add by haowei.cheng*/
	{
		port->current_pid = 0;
		memset(port->current_common,0,sizeof(port->current_common));
		strcpy(port->current_common,"");
		CCCI_ERROR_LOG(port->md_id, CHAR,"port %s close by %s,pid=%d,usage_cnt=%d\n",
			port->name,current->comm,current->pid,atomic_read(&port->usage_cnt));
	}
	/*END mipc port debug,add by haowei.cheng*/
...
void port_net_dump_fill(struct port_t *port, int dir, struct sk_buff *skb, char *param, int size)
{
	int type = 0;
	...
	if (dir == 0) {
		unsigned int pkt_type = skb->data[0] & 0xF0;
		if (pkt_type == 0x60)
	...
	if (skb->protocol == htons(ETH_P_IP)) {
		if (dir == 0)
			...
			iph = ip_hdr(skb);
		ret = snprintf(param, size,
			"net(%d):%d,%d, %s:(id:%x,src:%pI4,dst:%pI4,len:%d)",
			dir, port->flags, (dir == 0 ? port->rx_ch : port->tx_ch),
			(dir == 0 ? "R" : "W"), iph->id,
	...
	if (skb->protocol == htons(ETH_P_IPV6)) {
		...
		ret = snprintf(param, size,
			"net(%d):%d,%d, %s:(src:%pI6,dst:%pI6,len:%d,type:%d)",
			dir, port->flags, (dir == 0 ? port->rx_ch : port->tx_ch),
			(dir == 0 ? "R" : "W"),
			&ip6h->saddr, &ip6h->daddr, skb->len, type);
	}
	if (ret <= 0) {
		CCCI_ERROR_LOG(port->md_id, TAG, "generate port_net_dump fail\n");
	}

}

void port_net_dump(char *param, int md_id, int seq_num)
{
	u64 ts_nsec;
...
void port_ch_dump(struct port_t *port, int dir, void *msg_buf, int len)
{
	if (port->flags & PORT_F_DUMP_RAW_DATA)
		...
		CCCI_ERROR_LOG(-1, TAG,
			"not support port_ch_dump() for PORT_F_NET_DUMP\n");
	else
...
static inline int proxy_check_critical_user(struct port_proxy *proxy_p)
{
	// colgin @{
	//int ret = 1;
	int ret = 0;  // No need to wait critical user
	// @}
	...
	for (i = 0; i < ARRAY_SIZE(proxy_p->rx_ch_ports); i++) {
		list_for_each_entry(port, &proxy_p->rx_ch_ports[i], entry) {
			CCCI_NORMAL_LOG(proxy_p->md_id, TAG,
				"CH%d ports:%s(%d/%d) q(%d/%d),ee_q(%d/%d)\n",
	...
	for (hif = 0; hif < ARRAY_SIZE(proxy_p->queue_ports); hif++) {
		for (i = 0; i < ARRAY_SIZE(proxy_p->queue_ports[hif]); i++) {
			list_for_each_entry(port,
				...
				CCCI_NORMAL_LOG(proxy_p->md_id, TAG,
					"HIF%d, Q%d, ports:%s(%d/%d) q(%d/%d),ee_q(%d/%d)\n",
	...
	int hif, int qno, int dir, unsigned int state)
{
	...
	if (hif < 0 || hif >= MAX_HIF_NUM) {
		CCCI_ERROR_LOG(-1, CORE, "%s-%d: invalid hif = %d\n", __func__, __LINE__, hif);
		return;
	} else if (qno < 0 || qno >= MAX_QUEUE_NUM) {
		CCCI_ERROR_LOG(-1, CORE, "%s-%d: invalid qno = %d\n", __func__, __LINE__, qno);
		return;
	}
...
static void ccci_proc_init(void)
{
	...
	ccci_dbm_proc = proc_create("ccci_lp_mem", 0660, NULL, &ccci_dbm_ops);
