...
static void ccif_enable_irq(struct ccci_modem *md)
{
...
static void ccif_disable_irq(struct ccci_modem *md)
{
	struct md_sys1_info *md_info = (struct md_sys1_info *)md->private_data;
...
static void wdt_enable_irq(struct ccci_modem *md)
{
...
static void wdt_disable_irq(struct ccci_modem *md)
{
...
static irqreturn_t md_cd_wdt_isr(int irq, void *data)
{
	...
	if (!md) {
		CCCI_ERROR_LOG(-1, FSM, "%s, md NULL!!!\n", __func__);
		return IRQ_NONE;
	}

	mdss_dbg = ccci_md_get_smem_by_user_id(md->index,
	...
	CCCI_ERROR_LOG(md->index, TAG, "MD WDT IRQ:%x\n",
...
{
	CCCI_NORMAL_LOG(md->index, TAG, "EX CCIF HS state:%d\n", stage);

	switch (stage) {
	case HIF_EX_INIT:
		//ccci_ccif_exception(CCIF_HIF_ID, HIF_EX_INIT);
		//ccci_ccif_ex_handshake(md->index, ex_hif_id, HIF_EX_INIT);

		ccci_hif_dump_status(CCIF_HIF_ID,
			DUMP_FLAG_CCIF_REG| DUMP_FLAG_IRQ_STATUS, NULL, 0);
		ccci_hif_send_data(CCIF_HIF_ID, H2D_EXCEPTION_ACK);
		CCCI_NORMAL_LOG(md->index, TAG, "send H2D_EXCEPTION_ACK to MD\n");

		break;
	case HIF_EX_INIT_DONE:
		//ccci_ccif_exception(CCIF_HIF_ID, HIF_EX_INIT_DONE);
		//ccci_ccif_ex_handshake(md->index, ex_hif_id,
		//	HIF_EX_INIT_DONE);
		break;
	case HIF_EX_CLEARQ_DONE:
		//ccci_ccif_exception(CCIF_HIF_ID, HIF_EX_CLEARQ_DONE);
		//ccci_ccif_ex_handshake(md->index, ex_hif_id,
		//	HIF_EX_CLEARQ_DONE);
		...
		msleep(2000);  //give DHL some time to flush data
#else
		CCCI_NORMAL_LOG(md->index, TAG,
			"no wait for DHL flush data\n");
#endif

		ccci_hif_send_data(CCIF_HIF_ID, H2D_EXCEPTION_CLEARQ_ACK);
		CCCI_NORMAL_LOG(md->index, TAG, "send H2D_EXCEPTION_CLEARQ_ACK to MD\n");
		break;
	case HIF_EX_ALLQ_RESET:
		md->per_md_data.is_in_ee_dump = 1;
		//ccci_ccif_exception(CCIF_HIF_ID, HIF_EX_ALLQ_RESET);
		//ccci_ccif_ex_handshake(md->index, ex_hif_id,
		//	HIF_EX_ALLQ_RESET);
		break;
	default:
...
static int ccci_md_hif_start(struct ccci_modem *md)
{
...
#else
		CCCI_NORMAL_LOG(md->index, TAG, "skip MD ccci_md_clear_smem\n");
#endif
...
#else
		CCCI_NORMAL_LOG(md->index, TAG, "skip MD start_platform\n");
#endif
...
#else
	CCCI_NORMAL_LOG(md->index, TAG, "skip MD power_on\n");
#endif
...
#else
	CCCI_NORMAL_LOG(md->index, TAG, "skip MD let_md_go\n");
#endif

	wdt_enable_irq(md);
	ccci_md_hif_start(md);
...
static int md_cd_pre_stop(struct ccci_modem *md, unsigned int stop_type)
{
	...
	if (!per_md_data) {
		CCCI_ERROR_LOG(md->index, TAG, "ccci_get_per_md_data fail!\n");
	...
	if (stop_type == MD_FLIGHT_MODE_ENTER) {
		...
		if (pending) {
			...
			if ((md_dbg_dump_flag & (1 << MD_DBG_DUMP_SMEM)) && mdccci_dbg && mdss_dbg) {
...
static void debug_in_flight_mode(struct ccci_modem *md)
{
	...
	while (spm_is_md1_sleep() == 0) {
		count--;
		if (count == 0 && atomic_read(&md_dapc_ke_occurred)) {
			if (en_power_check) {
				CCCI_NORMAL_LOG(md->index, TAG,
				...
				"Dump MD EX log\n");
				if ((md_dbg_dump_flag &
					(1 << MD_DBG_DUMP_SMEM)) && mdccci_dbg && mdss_dbg) {
					ccci_util_mem_dump(md->index,
					CCCI_DUMP_MEM_DUMP,
					mdccci_dbg->base_ap_view_vir,
					mdccci_dbg->size);
...
static ssize_t md_cd_dump_show(struct ccci_modem *md, char *buf)
{
	...
	count = snprintf(buf, 256,
		"support: ccif register smem image layout\n");
	...
	const char *buf, size_t count)
{
	...
	if (md_state != GATED && md_state != INVALID) {
		...
		// AMMS_v3, DRDI MPU region is FORBIDDEN @{
		if (get_md_drdi_ver_from_lk() != AMMS_V3) {
	...
	const char *buf, size_t count)
{
	...
	} else if (strncmp(buf, "ccci_trm", count - 1) == 0) {
		CCCI_NORMAL_LOG(md->index, TAG, "TRM triggered\n");
		ccci_fsm_ioctl(md->index, CCCI_IOC_MD_RESET, 0);
	}
...
static ssize_t md_net_speed_show(struct ccci_modem *md, char *buf)
{
	...
	count = snprintf(buf, 4096, "curr netspeed log: %d\n",
...
static ssize_t md_cd_smem_show(struct ccci_modem *md, char *buf)
{
	...
	if (ret < 0) {
		CCCI_NORMAL_LOG(-1, TAG, "[%s] get md smem buf fail ret=%d\n",
			__func__, ret);
		return 0;
	}
...
// for datacard suspend testing
CCCI_MD_ATTR(NULL, control, 0660, md_cd_control_show,
	md_cd_control_store);
...
/* LG6851F Stage5N: use the kernel DMA_BIT_MASK() definition. */
	...
	struct ccci_dev_cfg *dev_cfg, struct md_hw_info *md_hw)
{
	...
	md->plat_dev->dev.dma_mask = &dpmaif_dmamask;
	md->plat_dev->dev.coherent_dma_mask = dpmaif_dmamask;
	md->ops = &md_cd_ops;
...
#if defined(CONFIG_MTK_ECCCI_FLASHLESS)
	ccci_fsm_ioctl(md->index, CCCI_IOC_DO_START_MD, NULL);
	CCCI_NORMAL_LOG(md->index, TAG,
		"%s: trigger CCCI_IOC_DO_START_MD internally\n", __func__);
#endif
