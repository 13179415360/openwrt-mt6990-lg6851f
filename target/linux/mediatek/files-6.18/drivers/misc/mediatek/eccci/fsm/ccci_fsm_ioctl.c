...
extern unsigned int aee_off_fast_reboot_enable;
#endif
...
static int fsm_md_data_ioctl(int md_id, unsigned int cmd, unsigned long arg)
{
	...
	if (per_md_data == NULL) {
		CCCI_ERROR_LOG(md_id, FSM, "ccci_get_per_md_data fail!\n");
		return -EFAULT;
	}
	...
	if (ret != 0) {
		CCCI_ERROR_LOG(md_id, FSM, "of_property_read_u32 fail!\n");
		return ret;
	}
	...
	case CCCI_IOC_GET_MD_PROTOCOL_TYPE:
		...
		ret = snprintf((void *)ap_platform, sizeof(ap_platform), "%d",
			md_gen);
		if (ret < 0 || ret >= sizeof(ap_platform)) {
			CCCI_ERROR_LOG(md_id, FSM,
					"%s-%d:snprintf fail,ret = %d\n", __func__, __LINE__, ret);
		}
	...
	case CCCI_IOC_DL_TRAFFIC_CONTROL:
		...
		switch (data) {
		case 1:
			/* Turn off downlink queue: not implemented in this port. */
			break;
		case 0:
			/* Turn on downlink queue: not implemented in this port. */
			break;
		default:
			/* Unsupported traffic-control value: no action. */
			break;
		}
		ret = 0;
		break;
...
long ccci_fsm_ioctl(int md_id, unsigned int cmd, unsigned long arg)
{
	...
	case CCCI_IOC_SEND_START_MD_REQUEST:
		...
		inject_md_status_event(md_id, MD_STA_EV_START_REQUEST,
					current->comm);
		break;
	case CCCI_IOC_SEND_RUNTIME_DATA:
#if !defined(CONFIG_MTK_ECCCI_FLASHLESS)
		aee_off_fast_reboot_enable = 1;
		CCCI_NORMAL_LOG(md_id, FSM, "set aee_off_fast_reboot_enable=%u\n", aee_off_fast_reboot_enable);
#else
		CCCI_NORMAL_LOG(md_id, FSM, "set aee_off_fast_reboot_enable not support\n");
#endif
		break;
	case CCCI_IOC_DO_START_MD:
#if defined(CONFIG_MTK_ECCCI_NO_MDINIT)
