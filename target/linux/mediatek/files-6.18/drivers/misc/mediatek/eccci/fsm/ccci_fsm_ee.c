	...
	const unsigned int type, const char *str)
{
	...
	int ret = 0;
#if IS_ENABLED(CONFIG_MTK_AEE_FEATURE)
	char *md_info_str = ccci_get_md_info_str(ee_ctl->md_id);
#endif
	...
	CCCI_NORMAL_LOG(ee_ctl->md_id, FSM, "%s\n",
		ee_ctl->ex_start_time);

#if IS_ENABLED(CONFIG_MTK_AEE_FEATURE)
	if (md_info_str != NULL && strstr(md_info_str, "Release")) {
		// mask pmic warm reset if modem product ver was "Release"
		mtk_aee_exception_notifier_call_chain(
			AEE_EXCEPTION_NOTIFY_TYPE_MD_EE, NULL);
	}
#endif
}
...
void fsm_md_bootup_timeout_handler(struct ccci_fsm_ee *ee_ctl)
{
	...
	CCCI_NORMAL_LOG(ee_ctl->md_id, FSM,
		"HS1 or HS2 timeout, proceed with reboot handler\n");
	fsm_reboot_handler();
#endif
}
...
void fsm_md_exception_stage(struct ccci_fsm_ee *ee_ctl, int stage)
{
	...
	if (stage == 0) { /* CCIF handshake just came in */
		...
		if (mem_layout == NULL) {
			...
			CCCI_ERROR_LOG(md_id, FSM, "ccci_md_get_modem_by_id fail!\n");
			return;
		} else if (mdss_dbg == NULL) {
			CCCI_ERROR_LOG(md_id, FSM, "ccci_md_get_smem_by_user_id fail!\n");
			return;
		}
