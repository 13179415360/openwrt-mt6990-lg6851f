	...
	int event_type, char reason[])
{
	...
	if (reason != NULL) {
		ret = scnprintf(user_ctlb->event_buf[user_ctlb->curr_w].reason, 32,
			"%s", reason);
		if (ret < 0 || ret >= 32) {
			CCCI_UTIL_DBG_MSG("%s-%d:scnprintf fail,ret=%d\n",
		...
		const struct timespec64 *time_stamp, int event_type, char reason[])
{
	...
	if (reason != NULL) {
		ret = scnprintf(last_md_status[md_id].reason, 32, "%s", reason);
		if (ret < 0 || ret >= 32) {
			CCCI_UTIL_DBG_MSG("%s-%d:scnprintf fail,ret=%d\n",
				__func__, __LINE__, ret);
		}
	} else {
		ret = scnprintf(last_md_status[md_id].reason, 32, "%s", "----");
		if (ret < 0 || ret >= 32) {
			CCCI_UTIL_DBG_MSG("%s-%d:scnprintf fail,ret=%d\n",
				__func__, __LINE__, ret);
		}
...
static int get_lock_rst_user_cnt(int md_id)
{
	if (md_id == 0)
...
static int get_lock_rst_user_list(int md_id, char list_buff[], int size)
{
	int cpy_size;
	int total_size = 0;
	struct ccci_util_bc_user_ctlb *user_ctlb = NULL;
