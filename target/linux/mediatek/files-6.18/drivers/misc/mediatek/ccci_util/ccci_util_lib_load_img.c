...
char *ccci_get_ap_platform(void)
{
	...
	if (!s_ap_platform_info) {
		node = of_find_compatible_node(NULL, NULL,
			"mediatek,mddriver");
		ret = of_property_read_u32(node, "mediatek,ap_plat_info",
			&ap_plat_info);
		if (ret < 0)
			return NULL;
		...
		ret = snprintf(s_ap_platform_info, 16, "MT%u", ap_plat_info);
		if (ret < 0 || ret >= 16) {
			CCCI_UTIL_ERR_MSG("%s: snprintf ap_platform fail with ret=%d\n", __func__, ret);
			kfree(s_ap_platform_info);
			s_ap_platform_info = NULL;
			return NULL;
		}
	}
	...
	if (head->header_verno >= 2) {
		...
		if (head->md_img_size) {
			if (image->size >= head->md_img_size)
				...
				ret = scnprintf(title, sizeof(title),
					"MD%d mem size smaller than image header setting",
					md_id + 1);
					if (ret < 0 || ret >= sizeof(title)) {
						CCCI_UTIL_INF_MSG("%s-%d:scnprintf fail,ret=%d\n",
							__func__, __LINE__, ret);
					}
				ret = scnprintf(info, sizeof(info),
...
TRY_LOAD_IMG:
	...
	while (1) {
		...
		start = ioremap((load_addr + read_size),
