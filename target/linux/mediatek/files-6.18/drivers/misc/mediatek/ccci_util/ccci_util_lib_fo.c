...
#include <linux/mm.h>
#include <linux/vmalloc.h> /* LG6851F Stage5C: vunmap declaration */
...
EXPORT_SYMBOL(is_cdma2000_enable);
...
static struct ThinMdDhl s_dhl_misc_info;
static void dhl_misc_info_parsing(void)
{
	memset(&s_dhl_misc_info, 0, sizeof(struct ThinMdDhl));
	if (find_ccci_tag_inf("dhl_misc_info", (char *)&s_dhl_misc_info,
			sizeof(struct ThinMdDhl)) != sizeof(struct ThinMdDhl)) {
		CCCI_UTIL_ERR_MSG("Invalid dhl_misc_info dt para\n");
	}
	return;
}

static int s_sar_idx;
static void sar_idx_parsing(void)
{
	s_sar_idx = 0;
	if (find_ccci_tag_inf("sar_idx", (char *)&s_sar_idx,
			sizeof(int)) != sizeof(int)) {
		CCCI_UTIL_ERR_MSG("Invalid sar_idx dt para\n");
	}
	return;
}

static int s_exhs2_reboot;
static void exhs2_reboot_parsing(void)
{
	s_exhs2_reboot = 0;
	if (find_ccci_tag_inf("exhs2_reboot", (char *)&s_exhs2_reboot,
			sizeof(int)) != sizeof(int)) {
		CCCI_UTIL_ERR_MSG("Invalid exhs2_reboot dt para\n");
	}
	return;
}

static int s_force_no_reboot;
static void force_no_reboot_parsing(void)
{
	s_force_no_reboot = 0;
	if (find_ccci_tag_inf("force_no_reboot", (char *)&s_force_no_reboot,
			sizeof(int)) != sizeof(int)) {
		CCCI_UTIL_ERR_MSG("Invalid force_no_reboot dt para\n");
	}
	return;
}

static struct ThinMdDrdiRuntimeData s_drdi_rt_data;
static void drdi_rt_data_parsing(void)
{
	memset(&s_drdi_rt_data, 0, sizeof(struct ThinMdDrdiRuntimeData));
	if (find_ccci_tag_inf("drdi_rt_data", (char *)&s_drdi_rt_data,
			sizeof(struct ThinMdDrdiRuntimeData)) != sizeof(struct ThinMdDrdiRuntimeData)) {
		CCCI_UTIL_ERR_MSG("Invalid drdi_rt_data dt para\n");
	}
	return;
}

static int s_is_host_fpga;
static void is_host_fpga_parsing(void)
{
	s_is_host_fpga = 0;
	if (find_ccci_tag_inf("is_host_fpga", (char *)&s_is_host_fpga,
			sizeof(int)) != sizeof(int)) {
		CCCI_UTIL_ERR_MSG("Invalid is_host_fpga dt para\n");
	}
	return;
}
#endif
...
// AMMS_v3, modem drei version
static int md_drdi_ver;
...
static void md_chk_hdr_info_parse(void)
{
	...
	if (s_g_md_usage_case & (1<<MD_SYS1)) {
		...
		// AMMS_v3, get modem drdi version
		if (find_ccci_tag_inf("drdi_version",
				(char *)&md_drdi_ver, sizeof(int)) != sizeof(int)) {
					md_drdi_ver = 0;
		}
		CCCI_UTIL_INF_MSG("ccci_util get drdi_version: %d\n", md_drdi_ver);
	}
_check_md3:
...
int __weak free_reserved_memory(phys_addr_t start_phys, phys_addr_t end_phys)
{
	...
	if ((end_phys - start_phys) < PAGE_SIZE ||
			(!pfn_valid(__phys_to_pfn(start_phys)) || !pfn_valid(__phys_to_pfn(end_phys - 1)))) {
		pr_notice("%s invalid start_phys:%pa end_phys:%pa with page_size=0x%lx\n",
				__func__, &start_phys, &end_phys, PAGE_SIZE);
		return -1;
	}

	pr_info("%s start_phys:%pa end_phys:%pa with page_size=0x%lx\n",
			__func__, &start_phys, &end_phys, PAGE_SIZE);
	// free memory in the manner of page unit
	for (pos = PAGE_ALIGN(start_phys);
			pos < end_phys && (end_phys - pos) >= PAGE_SIZE;
			pos += PAGE_SIZE, pages++)
		free_reserved_page(phys_to_page(pos));

	if (pages)
		pr_info("Freeing reserved memory: %ldK (%lu pages) from phys 0x%llx to phys 0x%llx\n",
				pages << (PAGE_SHIFT - 10), pages,
				(unsigned long long)PAGE_ALIGN(start_phys), (unsigned long long)pos);
...
static void dump_retrieve_info(void)
{
	...
	for (i = 0; i < retrieve_num; i++) {
		ret = snprintf(buf, 32, "retrieve%d", i);
		if (ret < 0 || ret >= 32) {
			CCCI_UTIL_ERR_MSG("%s-%d:snprintf fail,ret = %d\n", __func__, __LINE__, ret);
		}
...
#if defined(CONFIG_MTK_ECCCI_FLASHLESS)
	dhl_misc_info_parsing();
	sar_idx_parsing();
	exhs2_reboot_parsing();
	force_no_reboot_parsing();
	drdi_rt_data_parsing();
	is_host_fpga_parsing();
#endif
	parse_mpu_setting();
	md_mem_info_parsing();
	dump_retrieve_info();
...
#if defined(CONFIG_MTK_ECCCI_FLASHLESS)
struct ThinMdDhl get_dhl_misc_info(void)
{
	return s_dhl_misc_info;
}
EXPORT_SYMBOL(get_dhl_misc_info);

int get_sar_idx(void)
{
	return s_sar_idx;
}
EXPORT_SYMBOL(get_sar_idx);

int is_exhs2_reboot(void)
{
	return s_exhs2_reboot;
}
EXPORT_SYMBOL(is_exhs2_reboot);

int is_force_no_reboot(void)
{
	return s_force_no_reboot;
}
EXPORT_SYMBOL(is_force_no_reboot);

struct ThinMdDrdiRuntimeData get_drdi_rt_data(void)
{
	return s_drdi_rt_data;
}
EXPORT_SYMBOL(get_drdi_rt_data);

int is_host_fpga(void)
{
	return s_is_host_fpga;
}
EXPORT_SYMBOL(is_host_fpga);
#endif
...
static unsigned int get_md_smem_align(int md_id)
{
	return 0x4000;
}
...
// AMMS_v3 @{
int get_md_drdi_ver_from_lk(void)
{
	return md_drdi_ver;
}
EXPORT_SYMBOL(get_md_drdi_ver_from_lk);
// @}
/**************************************************************/
/* The following functions are back up for old platform       */
/**************************************************************/
static int __init ccci_parse_meta_md_setting(void)
{
	/* Device tree method */
	struct device_node *node = NULL;
	unsigned int *raw_ptr;
...
static void cal_md_settings_v2(struct device_node *node)
{
	...
	/* MTK_MD*_SUPPORT */
	for (i  = 0; i < MAX_MD_NUM_AT_LK; i++) {
		ret = snprintf(tmp_buf, 30, "opt_md%d_support", (i + 1));
		if (ret < 0 || ret >= 30) {
			CCCI_UTIL_ERR_MSG("%s-%d:snprintf fail,ret = %d\n", __func__, __LINE__, ret);
		}
	...
	/* MD*_SMEM_SIZE */
	for (i = 0; i < MAX_MD_NUM_AT_LK; i++) {
		ret = snprintf(tmp_buf, 30, "mediatek,md%d-smem-size", i+1);
		if (ret < 0 || ret >= 30) {
			CCCI_UTIL_ERR_MSG("%s-%d:snprintf fail,ret = %d\n", __func__, __LINE__, ret);
		}
	...
	/* MD1MD3_SMEM_SIZE*/
	ret = snprintf(tmp_buf, 30, "mediatek,md1md3-smem-size");
	if (ret < 0 || ret >= 30) {
		CCCI_UTIL_ERR_MSG("%s-%d:snprintf fail,ret = %d\n", __func__, __LINE__, ret);
	}
	...
	/* CFG version */
	ret = snprintf(tmp_buf, 30, "mediatek,version");
	if (ret < 0 || ret >= 30) {
		CCCI_UTIL_ERR_MSG("%s-%d:snprintf fail,ret = %d\n", __func__, __LINE__, ret);
	}
	tmp = 0;
...
static int ccci_reserve_mem_of_init(struct reserved_mem *rmem)
{
	phys_addr_t rptr = 0;
