...
static unsigned int ccci_log_poll(struct file *fp,
...
static unsigned int ccci_dump_poll(struct file *fp,
				   struct poll_table_struct *poll)
{
...
{
	volatile unsigned int *curr_p = (unsigned int *)start_addr;
	volatile unsigned char *curr_ch_p = NULL;
...
void ccci_log_init(void)
{
	...
	ccci_log_proc = proc_create("ccci_log", 0660, NULL, &ccci_log_fops);
