...
pdn_lock() {
    ...
    if [ "$?" != "0" ]; then
        exec 1022>"/var/lock/ql_datacall.lock"
        echo "check lock"
        flock 1022
        if [ "$?" != "0" ]; then
...
check_auto_apn_prov() {
    local configured_plmn sim_plmn
    ...
    # The stock config stores China Mobile as 46002 while the SDK APN DB
    # reports MCC 460 + three-digit MNC 002 as 460002.  They identify the
    # same PLMN.  Normalize the combined representation before deciding that
    # APN configuration changed; otherwise netifd enters an endless
    # setup/teardown loop and continuously forks mipc_wan_cli.
    configured_plmn="${1:-}"
    sim_plmn="${db_plmn:-}"
    case "$configured_plmn" in
        4600[0-9]) configured_plmn="4600${configured_plmn#460}" ;;
    esac
    echo "Config:$1 ($configured_plmn) & SIM:$db_plmn ($sim_plmn)"
    [ -n "$configured_plmn" ] && [ "$configured_plmn" = "$sim_plmn" ] && return

    uci set network."$2".proto='ql_datacall'
    uci set network."$2".device="ccmni"
    uci set network."$2".apn="$db_apn"
    uci set network."$2".auth="$db_auth_type"
    uci set network."$2".username="$db_user"
    uci set network."$2".password="$db_password"
    uci set network."$2".roamingtype="$db_roaming_protocol"
    uci set network."$2".iaroamingtype="$db_iaroaming_protocol"
    uci set network."$2".iptype="$db_protocol"
    uci set network."$2".iaapn="$db_iaapn"
    uci set network."$2".iaiptype="$db_iaprotocol"
    uci set network."$2".plmn="$db_plmn"
    uci set network."$2".dataroaming="$db_data_roaming"
    uci commit network
...
ql_update_mtu() {
    ...
    if [ $has_mtu -eq 0 ] ; then
        ...
        # The vendor reference image resets an MT753x/AN8855 switch here with
        # the proprietary `switch` utility and then renegotiates eth1.  The
        # LG6851F board instead exposes direct MT6990 GMAC/SGMII links, so that
        # sequence addresses hardware which is not present and must not be
        # copied to this target.  dnsmasq has already been reloaded above; new
        # DHCP leases receive the updated MTU option without a PHY reset.
        if [ $2 -eq 0 ] ; then
            echo "Skip vendor switch reset on direct-GMAC LG6851F topology"
        else
...
# Keep data-call setup behind the same userspace chain used by the vendor
# image.  rc.d priorities establish launch order, but netifd protocol workers
# run asynchronously and may otherwise grab the exclusive ttyCMIPC6 before
# the CCCI/MIPC daemons have actually opened their control ports.
wait_modem_stack_ready() {
    local waited=0
    local missing
    local md_boot
    ...
    while [ "$waited" -lt 60 ]; do
        missing=""
        md_boot="$(cat /sys/kernel/ccci/boot 2>/dev/null)"
        case "$md_boot" in md1:4*) ;; *) missing="$missing md-ready" ;; esac
        [ -e /dev/ttyCMIPC6 ] || missing="$missing ttyCMIPC6"

        for daemon in ccci_fsd ccci_mdinit ccci_rpcd_com mtk_netagent scd ql_netd ql_ril_service atcid; do
            pidof "$daemon" >/dev/null 2>&1 || missing="$missing $daemon"
        done

        if [ -z "$missing" ]; then
            echo "Official modem userspace chain ready after ${waited}s"
            return 0
        fi

        [ $((waited % 5)) -ne 0 ] || echo "Wait for official modem chain:$missing"
        sleep 1
        waited=$((waited + 1))
    done

    echo "Official modem userspace chain timeout:$missing"
    return 1
}

# This is a reference design by Mediatek
proto_ql_datacall_setup() {
    local interface="$1"
    local sim_state timeout retry imei

    sensitive_log=`uci get system.@system[0].enable_sensitive_log`

    echo "proto_ql_datacall_setup with $interface"
    ...
    [ -z "$timeout" ] && timeout="5"
    # Match the vendor SDK default: five data-call attempts, spaced by the
    # existing three-second recovery/check-IA interval below.
    [ -z "$retry" ] && retry="5"
    [ -z "$vlan" ] && vlan="0"
    ...
    # The stock B-slot starts mtk_netagent (S22), scd (S53), ql_netd and
    # ql_ril_service (S85), then atcid. Wait for their real processes and MD ready,
    # rather than assuming that rc.d launch order means service readiness.
    if ! wait_modem_stack_ready; then
        proto_notify_error "$interface" MODEM_STACK_NOT_READY
        return 1
    fi

    # ttyCMIPC6 is exclusive.  Bring RF up in this same serialized netifd
    # owner before waiting for SIM/APN; the vendor ordering queried SIM first
    # while mipc_wan.init attempted radio-on concurrently, creating a deadlock
    # (APN timeout vs. repeated EBUSY) on the OpenWrt procd startup sequence.
    echo "Enable modem radio before SIM/APN provisioning"
    ...
    do
        ...
        if [ "$check_sim_count" -lt "$timeout" ]; then
            ...
            # Vendor mipc.proto polls SIM readiness every five seconds.
            sleep 5;
        else
    ...
    [ ! -w /proc/bootprof ] || echo -n "modem camp on" > /proc/bootprof
    ...
    if [ ${mode} -eq "2" ] ; then
        ...
        ql_datacall_setup_bridge "$ifname" "$v4_addr" "$v4_mask" "$v4_gw" "$v4_dns1" "$v4_dns2" "$v4_mtu" "$v6_addr" "$v6_dns1" "$v6_dns2" "$odu_addr" "$odu_dhcp_offset" "$vlan"
    elif [ ${mode} -eq "3" ] ; then
        proto_init_update "$ifname" 1 1
        proto_set_keep 1
        ...
        ql_datacall_setup_bridge_with_nat "$ifname" "$v4_addr" "$v4_mask" "$v4_gw" "$v4_dns1" "$v4_dns2" "$v4_mtu" "$v6_addr" "$v6_dns1" "$v6_dns2" "$odu_addr" "$odu_dhcp_offset" "$vlan"
    else
        ...
        if [ -n "$clatd" ] ; then
            ...
            if [ -z $clad_if ] ; then
                ...
                uci set network.clatd.tunlink="$interface"
                uci commit network

                uci -q del_list firewall.@zone[1].network='clatd'
                uci -q add_list firewall.@zone[1].network='clatd'
...
proto_ql_datacall_teardown() {
    ...
    *)
    ...
    if [ ${mode} -eq 2 ] ; then
        ql_datacall_reset_bridge "$vlan"
    elif [ ${mode} -eq 3 ] ; then
        ql_datacall_reset_bridge_with_nat "$vlan"
    fi
}

ql_datacall_reset_bridge() {
    local section="lan"
    local eth_ifname="eth0"
...
ql_datacall_reset_bridge_with_nat() {
    local section="lan"
    local eth_ifname="eth0"
    local v4_addr
    local vlan="$1"
...
ql_datacall_setup_bridge_with_nat() {
    local ifname="$1"
    local v4_addr="$2"
    local v4_prefix="$3"
    local v4_gw="$4"
